...
**Dialog / overlay** (complemento, non quarta schermata): guscio unico `IngestDialogShell.jsx` (PR #377). Nuovi modal di revisione/split-view riusano la shell; dimensioni specifiche restano nel CSS del dialog figlio. **Visualizzatori documenti** (PDF / Word / Excel): chrome unico `DocumentViewerChrome` (Chiudi + Scarica + Schermo intero). Allegati Word/Excel: `InAppOfficeViewer`. Non è `IngestDialogShell`; `DocFileDialog` resta il form file.
