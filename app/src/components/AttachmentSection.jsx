 ...
 * Per le foto mostra PhotoEditModal prima del caricamento (rotazione + crop).
 *
 * Props:
...
function AttachmentSection({ questionId, attachmentManager, onUploadSuccess, customItemId = null }) {
  ...
  // ── Stato photo editor ──────────────────────────────────────────────────
  // photoEditQueue: null = modal chiuso, File[] = modal aperto con quelle foto
   ...
   * Per le foto apre prima PhotoEditModal; per documenti/verbali procede direttamente.
   */
  const handleUpload = async (category, source = "gallery") => {
    setShowUploadMenu(false);

    if (category === "foto") {
      // Apri file picker raw: i file vengono inviati al modal di editing
  ...
  /** Input change per le foto (hidden input ref) */
  const handlePhotoInputChange = useCallback((e) => {
    const files = Array.from(e.target.files || []);
    // Reset input per permettere riselezionare lo stesso file
    e.target.value = "";
    if (!files.length) return;
    setPhotoEditQueue(files);
  }, []);

  /** PhotoEditModal: tutte le foto editate → upload batch */
