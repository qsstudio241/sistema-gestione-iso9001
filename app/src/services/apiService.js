...
const API_CONFIG = {
    ...
    production: {
        baseUrl: import.meta.env.VITE_API_URL || 'https://sistemi.fr-busato.it:8443/api/v1',
        timeout: 15000
    }
};
...
class ApiService {
    ...
    async searchCompanyRegistry(data = {}, params = {}) {
        const query = new URLSearchParams(params).toString();
        return this.post(`/companies/registry/search${query ? '?' + query : ''}`, data);
    }
    ...
    async getRiskReviewsScope(params = {}) {
        const qs = new URLSearchParams(params).toString();
        return this.get(`/risks/reviews${qs ? `?${qs}` : ''}`);
    }
