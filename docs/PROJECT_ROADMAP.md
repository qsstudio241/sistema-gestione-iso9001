...
| **Meccanismo "Rielaborazioni disponibili" esteso alla WPQR (oggi solo Qualifiche)** | Verifica su richiesta committente 08/08/2026 ([GUIDA § 08/08/2026](GUIDA_CONSOLIDATA.md#lezioni-apprese-consolidate-fonte-unica)) | Il pannello superadmin (`reprocessableFields.js`, `qualificationReprocess.service.js`, `GET/POST /admin/reprocess-tasks`) è scritto solo per la tabella `qualifications` — nessun campo WPQR (`preheat_temp`, `interpass_temp`, `throat_test_mm`, `product_type`, `rotated_position`, `thickness_max_unlimited`) è recuperabile sui documenti già caricati senza ricaricare il PDF. Verificato 08/08/2026: solo **7 WPQR** in produzione, **4** con PDF ancora disponibile — volume troppo piccolo per giustificare la costruzione ora (correzione manuale via form "Modifica WPQR", già completo, in pochi minuti) | Costruire solo se il volume di WPQR cresce in modo significativo. Richiede generalizzare il registro a più tabelle (oggi `table: 'qualifications'` è hardcoded) — decisione di prodotto da prendere insieme: stesso pannello "Rielaborazioni disponibili" o sezione separata |
