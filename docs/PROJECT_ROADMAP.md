...
> **Prossimo Step**: **PR #38** aperta (`cursor/image-compress-word-resize-f765`, draft) — compressione foto upload, proporzionamento immagini Word, PhotoEditModal. Nessun deploy VPS necessario. **Azione committente**: merge PR #38 → `main` → Netlify auto-deploy (~2min). Pending L3 da committente: Smoke Mason ISO 3834 (passi 6-7); Smoke Word export Camellini; SMTP Alert Engine (variabili `.env` VPS).
> **Backlog**: norm_excerpt ISO 9001 (standard_id=1, backlog) | Export Word ISO 14001 / Rilievi Pendenti reali in Word / Modal Re-Audit lista pending (🔴) | Tabella "Rilievi Emersi" Word: aggiungere C e N.A. (da decidere) | ✅ GAP-B1/B2/B3 custom checklist (07/05) | ✅ migration 048/049/050 applicate | ✅ SYNC-5 allegati offline | ✅ compressione foto upload + PhotoEditModal (08/05, PR #38)
