...
|----|------|-------------------|----------|
| **AC-1** | **Guida UX conclusioni (opzionale)** | Verifica Camellini 06/05: conclusioni **già** nel report se compilate. `AuditClosePanel` già blocca export se vuote. Miglioramento opzionale: aprire Sezione 12 al focus sul pannello chiusura. | P2 — non bloccante |
| **AC-2** | **Report Word: layout e sommario** | Conclusioni OK in `wordExport.js`. Ridotti **pageBreak** ridondanti su ogni clausola checklist + spaziatori vuoti (`wordExportHelpers.js` 06/05). Verificare template `.docx` per marker `{conclusions}`. Tabella "Rilievi emersi" C/N.A. da confermare con cliente. | P1 |
