...
| **Controparti azienda (PR1 ✅)** | Tab **Controparti** in scheda azienda; `company_counterparties` (mig. **096**); backfill + `projects.end_customer_id` (mig. **097**); API nested; `contractReview` sync FK↔snapshot. **PR2:** select committente in UI riesame ([DEPUTYTASK](agent-tasks/DEPUTYTASK.md)). | Mig. 096–097; `companyCounterparties.controller.js`; `commercialCustomerCounterparty.service.js`; `CompanyCounterpartiesPanel.jsx` |
...
| P5 | **Deputy Mason — audit 2ª parte + fornitori anagrafica** (`fornitoreSupplierId`, `GET /suppliers?company_id=`, UI Anagrafiche `company_id`; parallelo PR #110) + foto Word OOXML fix (slice separata) | Deputy | 🔄 In corso — [`DEPUTYTASK.md`](agent-tasks/DEPUTYTASK.md) |
