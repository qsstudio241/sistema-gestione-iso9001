...
| **Range di qualificazione automatico WPQR/patentino (Tabelle 7/8/9 ISO 15614-1 e 6/7/8 ISO 9606-1)** | Gap analysis ingest 26/07/2026 ([GUIDA § Gap analysis ingest WPQR/qualifiche](GUIDA_CONSOLIDATA.md#lezioni-apprese-consolidate-fonte-unica)) | Calcolo automatico spessore/diametro qualificato da spessore/diametro testato richiederebbe codificare le tabelle normative — ma gli estratti `docs/reference/ISO-15614-1-range-validita-WPQR.md` e `ISO-9606-1-range-validita-patentino.md` segnalano diverse righe come **GAP** (cifre perse nell'estrazione PDF, non verificabili senza copia ufficiale) | Solo dopo verifica manuale delle tabelle su copia ufficiale della norma (non in Patrimonio Studio come testo strutturato); nel frattempo l'estrazione riporta solo il valore/range **esplicitamente scritto** sul documento, mai calcolato |
...
| **Scheduler automatico rielaborazione campi ingest (`reprocess-qualifications.js` periodico)** | Richiesta committente 28/07/2026 — backfill `transfer_mode` completato manualmente (15 proposte in coda) | **Decisione di costo, non tecnica**: ogni rielaborazione richiama l'AI (costo reale); uno scheduler automatico su tutto il DB a intervalli fissi genererebbe chiamate AI ricorrenti non supervisionate. Lo script generico esiste ma resta a **lancio manuale on-demand** (vedi [GUIDA § Rielaborazione/backfill](GUIDA_CONSOLIDATA.md#lezioni-apprese-consolidate-fonte-unica)) | Solo su **conferma esplicita del committente** sul trade-off costo/comodità (es. cron mensile `node scripts/reprocess-qualifications.js --field=<tutti i campi mancanti>`); implementazione stimata: job `node-schedule` in `alertScheduler.js` + log esito, nessuna nuova migrazione |
...
| **REGISTRO-LEGALE** | Registro obblighi legali capitolo-per-capitolo, **ambiente e sicurezza separati** (`LEG_AMBIENTE_152` esistente, nuovo `LEG_SICUREZZA_81` P0), sotto-domande SI/NO/NA con riferimenti legislativi narrativi su `custom_checklist_sections` (2 colonne nullable, no nuove tabelle). Estensione (non duplicazione) dell'agente `normValidityChecker.service.js` già in produzione (PR #65) per segnalare atti abrogati/sostituiti citati nel registro. **ADR-019** + 4 brief paralleli: [DEPUTYTASK1](agent-tasks/DEPUTYTASK1.md) (schema BE), [DEPUTYTASK2](agent-tasks/DEPUTYTASK2.md) (render FE), [DEPUTYTASK3](agent-tasks/DEPUTYTASK3.md) (contenuto sicurezza P0), [DEPUTYTASK4](agent-tasks/DEPUTYTASK4.md) (agente validità). | Lead (28/07/2026) | 🔲 ADR + brief pronti — implementazione da avviare |

**Prossimo Step**: COMPANY-PROFILE S1–S3 (migration + API/UI + import Excel) dopo merge S0; poi eventuale S5 lookup Registro. REGISTRO-LEGALE: avviare DEPUTYTASK1+2 in parallelo, poi DEPUTYTASK3 (contenuto P0), poi DEPUTYTASK4.
