...
> **Prossimo Step**: **ADR-009 Fase 1 in PR draft #40** — registro standard centralizzato (`app/src/data/standardsRegistry.js`) + metriche per-norma (`metrics.byStandard[key]`) + chip UI riusabile (`MetricsByStandardChip`). Branch `cursor/adr009-fase1-registro-standard-52c5`. DoD rispettata, 31/31 test L1 nuovi PASS, build pulita, zero modifiche backend. **Verifica committente**: dopo merge + deploy Netlify, controllare in produzione che la sidebar audit ISO 9001+14001 mostri "9001: N NC · 14001: M NC · totale K NC" come da DoD. Se OK in 24-48h → procedere con Fase 2 (sezione 11 + close panel per-norma + flag SGI integrato). **Azioni committente pendenti**: nessuna nuova; restano in osservazione gli audit Camellini in produzione (stabilità post-fix 08/05).
> **Backlog**: 🟡 ADR-009 Fase 2-5 (sezione 11 per-norma, export Word per-norma, custom checklist come norma virtuale, tie-in document_registry) | ✅ ADR-009 Fase 1 (PR #40 draft, 10/05/2026) | ✅ Fix CORS nginx fallback backend down (08/05/2026) | ✅ Fix licenze admin/superadmin bypass requireLicensedModule (08/05/2026) | ✅ Fix Exception 1 campi testo si svuotano (08/05/2026) | ✅ Fix Exception 4 multi-standard (08/05/2026) | ✅ Fix race rendering checklist multi-device (PR #39, 08/05/2026) | ✅ GAP-B1/B2/B3 custom checklist (PR #37, 08/05/2026) | Tabella "Rilievi Emersi" Word: aggiungere C e N.A. (da decidere con cliente) | norm_excerpt ISO 9001 (standard_id=1, backlog) | ✅ SYNC-5 allegati offline | ✅ migration 048-049-050 applicate
