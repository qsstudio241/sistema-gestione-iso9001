...
> **Prossimo Step**: **ADR-010 accettato** ([docs/adr/ADR-010-ai-agentic-architecture.md](adr/ADR-010-ai-agentic-architecture.md)) — architettura AI agentica: NormBroker multi-source, riesame requisiti assistito (§8.2), RAG incrementale (Gemini Flash gratis → pgvector), adapter multi-provider, licenze AI modulari. 4 fasi implementative con task paralleli per Cursor multitask. **Fase 0** (fondazione) avviabile subito con 4 task indipendenti. | **ADR-009 accettato** ([docs/adr/ADR-009-multi-standard-architettura-per-norma.md](adr/ADR-009-multi-standard-architettura-per-norma.md)) — architettura multi-standard / multi-document_type per-norma + AI-ready. 5 fasi implementative pianificate. **Prerequisito Fase 1**: stabilità conclamata (24-48h) del fix Exception 4 multi-standard mergiato 08/05 18:55 UTC. **Stato fix consecutivi 08/05/2026** tutti mergiati e deployati Netlify: PR #37 (3 GAP custom checklist) · PR #39 (race rendering checklist Sighinolfi) · fix campi testo si svuotavano · fix risposte sovrascritte da reconcile · fix Exception 4 per multi-standard · fix CORS quando backend in restart (config nginx) · fix licenze admin/superadmin bypass. **Azioni committente pendenti**: (1) Camellini riprova SIGHINOLFI e altri audit in campo (verifica stabilità); (2) ~~Smoke Mason ISO 3834 passi 6-7 (L3)~~ ✅ Verificato da Mason (10/05/2026); (3) ~~Compilare SMTP vars nel `.env` VPS + `ALERT_ENABLED=true` + restart~~ ✅ Fatto e verificato con e-mail di test (10/05/2026). **Prossima priorità agent**: ADR-009 Fase 1 (registro standard centralizzato + metriche per-norma) — solo dopo conferma stabilità; sblocca poi Export Word ISO 14001, Rilievi Pendenti reali in Word, Modal Re-Audit.
> **Backlog**: 🔴 ADR-009 Fase 1-5 (multi-standard/document_type/AI-ready) | ✅ Fix CORS nginx fallback backend down (08/05/2026) | ✅ Fix licenze admin/superadmin bypass requireLicensedModule (08/05/2026) | ✅ Fix Exception 1 campi testo si svuotano (08/05/2026) | ✅ Fix Exception 4 multi-standard (08/05/2026) | ✅ Fix race rendering checklist multi-device (PR #39, 08/05/2026) | ✅ GAP-B1/B2/B3 custom checklist (PR #37, 08/05/2026) | Tabella "Rilievi Emersi" Word: aggiungere C e N.A. (da decidere con cliente) | norm_excerpt ISO 9001 (standard_id=1, backlog) | ✅ SYNC-5 allegati offline | ✅ migration 048-049-050 applicate
> **Riferimenti**: [docs/GUIDA_CONSOLIDATA.md](GUIDA_CONSOLIDATA.md) (esperienza operativa) | [docs/adr/ADR-009-multi-standard-architettura-per-norma.md](adr/ADR-009-multi-standard-architettura-per-norma.md) (vincolante per multi-standard / nuovi document_type) | [docs/adr/ADR-010-ai-agentic-architecture.md](adr/ADR-010-ai-agentic-architecture.md) (vincolante per AI agentica: NormBroker, riesame assistito, RAG, licenze AI) | [docs/adr/ADR-008-event-sourcing-sync.md](adr/ADR-008-event-sourcing-sync.md) (sync event-based) | [docs/adr/ADR-006-auto-reconcile-cache-sync.md](adr/ADR-006-auto-reconcile-cache-sync.md) | [docs/DATABASE_SCHEMA.md](DATABASE_SCHEMA.md) (schema DB)
...
| **ADR-009 Fase 1** | Registro standard centralizzato + metriche per-norma (chip "NC: 2 (9001) · 1 (14001)" in sidebar) | Deputy/Lead | ⏳ Dopo conferma stabilità Camellini |
| **ADR-009 Fase 2** | Sezione 11 e Close Panel per-norma + flag SGI integrato | Deputy/Lead | ⏳ Dopo Fase 1 |
...
| **ADR-010** | **Architettura AI Agentica** — NormBroker multi-source, riesame requisiti assistito (§8.2), RAG incrementale, adapter multi-provider (Gemini/Azure/OpenAI), licenze `ai_assist`/`ai_norms`/`ai_review`/`ai_chat`. Task paralleli strutturati per Cursor multitask. | Lead (12/05/2026) | ✅ ADR scritto |
| **ADR-010 Fase 0** | **Fondazione AI**: adapter multi-provider (Gemini default), tabella `norm_requirements` + import norme locali, licenze AI in `KNOWN_MODULE_KEYS`, tabella `ai_interactions` audit trail, migrazione `importAiExtraction` su adapter | Multi-agente (4 task paralleli) | ⏳ Prossimo step |
| **ADR-010 Fase 1** | **Riesame requisiti agentico + RAG light**: NormBroker + connettori (locale, web, pubblico), API riesame contratto (Sprint 11 + AI), AI context builder + endpoint `/ai/suggest`, UI riesame con checklist pre-compilata, WebScraper per siti normativi | Multi-agente (5 task paralleli) | ⏳ Dopo Fase 0 |
| **ADR-010 Fase 2** | **Gap analysis + legislazione**: engine gap analysis clausola per clausola, matrice norme↔legislazione, connettore Normattiva.it, UI dashboard conformità | Multi-agente (4 task paralleli) | ⏳ Dopo Fase 1 |
| **ADR-010 Fase 3** | **RAG completo + chat** (dopo finanziamento): vector store pgvector, pipeline indicizzazione, assistente conversazionale con function calling, apprendimento per azienda | Multi-agente | ⏳ Dopo finanziamento |

**Prossimo Step**: **ADR-010 Fase 0** (Fondazione AI) — 4 task paralleli eseguibili da agenti Cursor indipendenti. Vedi [ADR-010](adr/ADR-010-ai-agentic-architecture.md) sezione "Piano di implementazione" per brief di ogni task. In parallelo, ADR-009 Fase 1 (registro standard + metriche per-norma) resta attivabile appena confermata stabilità Camellini.
...
> **Regola architetturale da ADR-010 (vincolante)**: ogni feature AI deve usare `aiProviderAdapter` (mai chiamate dirette a OpenAI/Gemini), loggare in `ai_interactions`, e rispettare le licenze modulari `ai_*`. Ogni nuova fonte normativa = 1 nuovo connector nel NormBroker, zero modifiche al codice applicativo. L'AI suggerisce, l'utente decide (human-in-the-loop obbligatorio per conformità ISO 9001 §7.5).
