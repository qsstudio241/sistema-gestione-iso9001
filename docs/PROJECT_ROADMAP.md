...
**Ultimo aggiornamento di questa sezione**: 13/08/2026 (ripristino upload batch Qualifiche + Ambito unico + harness).
...
Ambito azienda **unico** in alto (`CompanyScopeSelect` nell'header): vale su tutta l'app finché l'utente non lo cambia. I selettori di pagina sono **eliminati** (non spostati); le **azioni** di pagina si conservano. Default: admin/personale studio → Tutto lo studio; utente di un'azienda → quella azienda, non modificabile.
...
| **COMPANY-PROFILE** | Profilo azienda 1:1 (`company_profile`) per conformità legislativa 14001/45001: campi A (visura/Excel) + B (SSL/ambiente, inserimento studio); gate `SAL_LEGAL_CONFORMITY`; import Excel tipo scadenziario. **S0–S3 ✅** (mig. 145, GET/PUT, tab UI, import Excel detect→dry-run→upsert). Resta **S4** completeness/sync anagrafica e **S5** lookup registro (da rivedere; iCRIBIS è sito, non API). | Lead (23/07/2026) · S3 Deputy 13/08/2026 | 🔲 S3 fatto — resta S4/S5 |
...
**Prossimo Step**: COMPANY-PROFILE S4 (badge completezza + sync opzionale verso `companies`). REGISTRO-LEGALE: solo N5 (lettura consulente) e backlog P2 ambiente a/b/c. Lookup iCRIBIS/InfoCamere **non** in questa fase.
