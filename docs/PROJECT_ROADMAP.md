...
| **COMPANY-PROFILE** | Profilo azienda 1:1 (`company_profile`) per conformità legislativa 14001/45001: campi A (visura/Excel) + B (SSL/ambiente, inserimento studio); gate `SAL_LEGAL_CONFORMITY`; import Excel tipo scadenziario. **S0 ✅** ADR-018 + catalogo. **S1 ✅** mig. 145. **S2 ✅** API GET/PUT + tab UI. **S3** import Excel in [DEPUTYTASK.md](agent-tasks/DEPUTYTASK.md). | Lead (23/07/2026) · S2 Deputy 13/08/2026 | 🔲 S2 fatto — resta import Excel |
...
**Prossimo Step**: COMPANY-PROFILE S3 (import Excel) dopo S2 API/UI. REGISTRO-LEGALE: solo N5 (lettura consulente) e backlog P2 ambiente a/b/c.
