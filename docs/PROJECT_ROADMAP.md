...
| **REGISTRO-LEGALE** | Registro obblighi legali capitolo-per-capitolo, **ambiente e sicurezza separati** (`LEG_AMBIENTE_152` esistente, nuovo `LEG_SICUREZZA_81` P0), sotto-domande SI/NO/NA/NV con riferimenti legislativi narrativi su `custom_checklist_sections` (mig. **138**, 2 colonne nullable). Estensione (non duplicazione) dell'agente `normValidityChecker.service.js` già in produzione (PR #65). **ADR-019** · stream BE/FE/contenuto/agente su branch `cursor/registro-legale-adr019-77df` (PR #317). | Lead (28/07/2026) | 🔲 PR #317 aperta — revisione umana contenuto (N5) prima del merge su `main` |

**Prossimo Step**: COMPANY-PROFILE S1–S3 (migration + API/UI + import Excel) dopo merge S0; poi eventuale S5 lookup Registro. REGISTRO-LEGALE: revisione N5 contenuto + merge PR #317; poi mig. 138 su VPS.
