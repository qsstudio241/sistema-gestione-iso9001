...
- **Verifica post-deploy**: `GET https://busato.selfip.com:8443/api/v1/health` e, per feature nuove, l’endpoint specifico (es. `GET /api/v1/organizations/me` con Bearer → 401 senza token è normale; 404 = route/file non deployati).
...
- [ ] **Verifica rapida API**  
  `GET https://busato.selfip.com:8443/api/v1/health` → 200. Per smoke mirati usare anche l’endpoint della feature (con token se serve).
...
- [ ] **Variabile d’ambiente produzione**  
  In Netlify: Site → Settings → Environment variables → verifica che **VITE_API_URL** sia impostata con l’URL del backend di produzione (es. `https://busato.selfip.com:8443/api/v1`). Senza questa variabile il frontend in produzione non troverà le API.
