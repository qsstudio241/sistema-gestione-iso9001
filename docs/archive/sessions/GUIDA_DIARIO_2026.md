...
#### Ops/Sysadmin — Rinnovo SSL Let's Encrypt `sistemi.fr-busato.it` (diagnosi HTTP-01)

Recuperato dalla PR #28 (chiusa, contenuto consolidato qui).

| Evidenza | Dettaglio |
|----------|-----------|
| Scadenza cert | `notAfter=May 5 11:32:22 2026 GMT` su `https://sistemi.fr-busato.it:8443` |
| Sintomo `certbot renew` | Let's Encrypt risponde **HTTP-01 unauthorized**: **404** su `http://sistemi.fr-busato.it/.well-known/acme-challenge/...` |
...
| Trappola `/etc/hosts` | Rimuovere eventuale `127.0.0.1 sistemi.fr-busato.it` sul VPS (fa risolvere il dominio in loopback; backup `/etc/hosts.bak`). Da solo **non** risolve il 404 esterno. |
| **Porta 10880 (Nginx VPS)** | Virtual host dedicato `acme-challenge-10880.conf` (`sites-available` + symlink `sites-enabled`): **listen 10880**, serve solo `/.well-known/acme-challenge/` da `root /var/www/html`, resto **404**. Verifica da Internet: `curl -s http://sistemi.fr-busato.it:10880/.well-known/acme-challenge/probe-10880` deve dare `probe-10880`. |
| **Renewal config** | In `/etc/letsencrypt/renewal/sistemi.fr-busato.it.conf`: `authenticator = webroot`, `webroot_path = /var/www/html` (niente plugin nginx al renew; backup `.bak.<timestamp>`). |
...
Quando da rete esterna `curl -s http://sistemi.fr-busato.it/.well-known/acme-challenge/probe-10880` restituisce `probe-10880` (non un **301** Apache verso HTTPS): sul VPS `sudo certbot renew --force-renewal` poi `sudo systemctl reload nginx`. Verifica date: `echo | openssl s_client -connect 127.0.0.1:8443 -servername sistemi.fr-busato.it 2>/dev/null | openssl x509 -noout -dates`.
...
Verifica REST cross-studio su API produzione (`sistemi.fr-busato.it:8443`). Login da `.cursor/mcp.env` (`SGQ_APP_EMAIL` / `SGQ_APP_PASSWORD`) — **non** modifica hash password admin.
...
| 5 | Modificare **una** voce (esito + nota/evidenza se richiesta) e attendere autosalvataggio | Indicatore salvataggio ok o assenza errori bloccanti | **DevTools → Rete**: una richiesta verso API `sistemi.fr-busato.it` con **2xx** (non 401/423/404 ripetuti). |
...
**URL app:** https://systemgest.netlify.app/nc | **API:** https://sistemi.fr-busato.it:8443/api/v1
...
Usare **sempre** `127.0.0.1:11043` invece di `sistemi.fr-busato.it:11043` nei runner VPS — l'IP pubblico è bloccato da hairpin NAT. Il servizio systemd usa invece il nome host perché ha route diverse.
...
**Lezione chiave — SQL Server + VPS SSH:** il server SQL non è raggiungibile via hostname pubblico (`sistemi.fr-busato.it:11043`) dal VPS stesso — usare sempre `localhost:11043` negli script di migrazione eseguiti via SSH sul nodo.
