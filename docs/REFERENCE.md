...
Port HTTPS: 8443
API Base: https://busato.selfip.com:8443/api/v1
...
Deploy / API / SSH in un solo punto per gli agenti: [how-to/deploy.md](how-to/deploy.md) → [ACCESSO_DEPLOY_AGENTS.md](how-to/ACCESSO_DEPLOY_AGENTS.md).
...
**SSMS (schema tipico):**
```
Server=busato.selfip.com,11043;Database=SGQ_ISO9001;Integrated Security=False;User Id=<LOGIN_SQL>;Password=<SEGRETO>;
```
...
- **Deploy non interattivo sul VPS:** chiave SSH, **Pageant**, sessione **PuTTY** (`SGQ_PUTTY_SESSION`), file **`backend/config/.ssh-deploy.local.ps1`** (vedi [ACCESSO_DEPLOY_AGENTS.md](how-to/ACCESSO_DEPLOY_AGENTS.md)). Prompt password / host key: vedi [DEPLOY_TROUBLESHOOTING.md](how-to/DEPLOY_TROUBLESHOOTING.md).
- Fonte operativa DB: [DATABASE.md](reference/DATABASE.md).
...
```bash
ssh spascarella@busato.selfip.com -p 1122 "cd /var/www/sgq-backend && pkill -f 'node src/server.js' && nohup node src/server.js > /dev/null 2>&1 &"
```
...
```bash
scp -P 1122 backend/src/controllers/audit.controller.js spascarella@busato.selfip.com:/var/www/sgq-backend/src/controllers/
```

**Transfer Multiple Files:**
```bash
scp -P 1122 -r backend/src/services/* spascarella@busato.selfip.com:/var/www/sgq-backend/src/services/
```
