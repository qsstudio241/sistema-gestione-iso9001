# Playwright ù contesto licenze, RBAC azienda e billing
...
**Correlati**: [ARCHITETTURA_UTENTI_RBAC.md](../ARCHITETTURA_UTENTI_RBAC.md) ù [PROJECT_ROADMAP.md](../PROJECT_ROADMAP.md) ù migration `081_user_company_access.sql` ù migration `082_billing_layer.sql`

---

## A. Gerarchia e chi fa cosa

| Ruolo | Chi ù | Cosa DEVE poter fare | Cosa NON DEVE vedere/fare |
|-------|--------|----------------------|---------------------------|
| **`superadmin`** | Piattaforma QS Studio | Tutti i tenant, `PATCH /admin/licenses`, dashboard `/settings/billing` | ù (fuori scope test azienda) |
...
**Regola azienda (Fase 4)**: se `user.company_access.length > 0`, lùutente ù **cliente azienda** ù menu ridotto, link ùLa mia Aziendaù ? `/companies/{id}`.
...
| Modulo **assente** | **403** `{ code: "MODULE_NOT_LICENSED", module: "..." }` | Schermata `<ModuleLocked lockedByLicense />` ù badge ùNon incluso nel pianoù |
| `licensed_modules` **NULL / vuoto / JSON invalido** | Tutti i moduli noti abilitati (retrocompat) | Tutte le route licenziate visibili |

**Bypass licenze API**: solo `superadmin` e `admin` org ù **non** si applica al viewer azienda.
...
`/documents` ù `/qualifiche` ù `/nc` ù `/rischi` ù `/reclami` ù `/anagrafiche` ù `/contract-reviews` ù `/ai-assistant` ù `/ai-knowledge-health` ù `/sal` ù `/saldatura/*` ù `/settings/notifications` ù `/settings/import-jobs`
...
**Tabella**: `user_company_access` ù `(user_id, company_id, permission read|write, organization_id)`.
...
|--------|------|----------|
| Viewer `permission: read` | Vedere ´La mia Aziendaª, aprire scheda azienda assegnata, UI personale in sola lettura | Creare/modificare personale (POST ? **403**), menu studio/admin, mutazioni API su doc/qualifiche/rischi |
| Cliente `permission: write` | Come sopra + CRUD operativo sulla propria azienda | Gestire altre aziende, settings admin |
| Viewer senza riga in `user_company_access` | Comportamento legacy studio (se ha `auditor_org_id`) | ó |
...
- **Fatturabilitù** (backend): `is_billable = (billing_status === 'active' AND company.is_active === 1)` ù usata solo da snapshot/export superadmin.
- **`billing_events`**: scritti dal backend (creazione azienda, toggle attivo, backfill, aggiornamento licenze) ù **nessuna UI** per utente azienda.

### Cosa lùutente azienda NON DEVE vedere

- Voce menu **ùFatturazioneù** (`/settings/billing`) ù solo `superadmin`
- Chiamate API `GET /admin/billing/*` ù **403** (o 401) per non-superadmin
- Eventi billing, export CSV, riepilogo tenant/studi

**Nota commerciale** ([PROJECT_ROADMAP](../PROJECT_ROADMAP.md)): canone per azienda attiva + moduli abilitati; il cliente azienda **non** gestisce il proprio piano ù lo fa QS Studio via superadmin.
...
- **DEVE**: redirect home, sidebar con ùLa mia Aziendaù (non elenco studi)
- **DEVE**: URL azienda `/companies/11`
- **NON DEVE**: voci ùUtentiù, ùLicenze moduliù, ùFatturazioneù, ùIl mio Studioù

### 2. Modulo non licenziato

- **Setup**: tenant con `licensed_modules` che **esclude** es. `"nc"` (superadmin path separato ù solo menzione)
- **When**: viewer naviga a `/nc` o chiama API NC
- **DEVE (UI)**: titolo modulo + testo ùNon incluso nel pianoù / ùChiedi a un amministratoreùù
- **DEVE (API)**: **403** + `code: MODULE_NOT_LICENSED`

### 3. Navigazione moduli licenziati

- **Given**: tenant con moduli base (`documents`, `nc`, ù) in licenza
- **DEVE**: aprire `/documents`, `/nc` senza schermata locked
- **DEVE**: sidebar mostra voci corrispondenti

### 4. Write vs read (personale azienda)

- Viewer read ? pulsante ùAggiungi personaleù assente o disabilitato; POST personnel **403**
- Cliente write (`cliente.azienda11@ù`) ? POST personnel **201**

### 5. Superadmin (solo smoke separato)

- Path `/settings/billing`, `GET /admin/billing/overview` ù **fuori** suite ùutente aziendaù

---

## F. Dati test utili

### Account (VPS / locale ù password in `mcp.env`, non in repo)
...
| * | `/nc/*`, `/documents/*`, ù | **403** se modulo non in licenza |
...
`/settings/users` ù `/settings/licenses` ù `/settings/billing` ù `/settings/studio`
...
| `OPENAI_API_KEY` | Funzioni AI ù senza chiave alcune API rispondono **503** (non confondere con 403 licenza) |
| `licensed_modules` NULL | Fail-open: tutti i moduli ù impostare JSON esplicito per test ùmodulo bloccatoù |
