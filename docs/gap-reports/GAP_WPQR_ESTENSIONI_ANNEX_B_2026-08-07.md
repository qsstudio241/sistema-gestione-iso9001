...
| # | Item | Esito |
|---|---|---|
| 1 | Gola/throat FW | **Parzialmente chiuso**: aggiunto campo dichiarato `throat_test_mm` (schema FE+BE, colonna `wpqr_records.throat_test_mm`, migrazione 141) — prima non era estratto affatto. **Resta aperto**: non ancora usato in `wpsGenerator.service.js` per il calcolo copertura, perché `generateWpsFromWpqr`/l'API "Genera WPS" non hanno un parametro "gola richiesta" per il giunto FW in produzione — richiede una modifica più ampia (nuovo parametro richiesta + UI) fuori scope di questo fix mirato. |
