...
**Non confondere:** scadenza del documento ? regola di reminder email ? priorit� UI/roadmap.
...
|---------|------------------|-----------|---------|
| **1 � Scadenza (dato)** | Data fine validit� del singolo documento | `document_registry.expiry_date` (+ calcolo da `doc_type_config.default_expiry_months` al rilascio) | Procedura PG-012 scade il 15/09/2026 |
...
**Priorit� roadmap (P0/P1/P2):** backlog prodotto in `PROJECT_ROADMAP.md` � **non** mappare su colonne DB alert.
...
| **Responsabile documento** | `doc_notify_responsible` invia solo se `responsible` **� un indirizzo email** (non lookup personale/rubrica) |
...
- Roadmap priorit� prodotto: `docs/PROJECT_ROADMAP.md`
