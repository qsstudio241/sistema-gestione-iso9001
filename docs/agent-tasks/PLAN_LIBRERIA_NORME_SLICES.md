...
> **Destinazione**: in Gestione, una schermata studio **Libreria** con **due sezioni chiare** — (1) catalogo fonti di riferimento **ingerite** (norme, libri, quaderni, altri riferimenti) con **stato di validità/vigore** dove applicabile (qualità info per agenti/AI); (2) **richieste mancanti** per aumentare affidabilità risposte. Riuso massimo (Gate Ponytail / ADR-011): niente secondo inventario.
>
...
> **Mappa creata**: 29/08/2026 · Lead proposta UI punto 3 Quaderno / qualità · aggiornata decisione nome/scope
...
- **No** nuovo SoT: sezione Catalogo legge `document_registry` + `validity_status` (su `norma` in `type_specific_data`, ADR-011)
...
|---------|------------------------|------------|
| **1. Catalogo ingerito** | Elenco fonti di riferimento già in Registro (norme e, se tipizzate, altri riferimenti) con codice/titolo/edizione e badge vigore dove applicabile | DB `document_registry` (filtro tipi già tipizzati; LN-1 partenza `norma`) |
...
|-------|------|-----------|------------|------|-------|
| **LN-1** | Shell Gestione → **Libreria** + 2 sezioni read-only | FE: pagina + voce menu; riuso API documenti filtro tipi riferimento già tipizzati; render backlog (parse MD o JSON statico generato da MD) | Nome confermato | AFK | brief APERTO |
