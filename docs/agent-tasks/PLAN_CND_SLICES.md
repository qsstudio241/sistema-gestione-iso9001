...
> **Brief attivi**: [`DEPUTYTASK1.md`](DEPUTYTASK1.md) **CND-W APERTO** (export Word PT/MT). [`DEPUTYTASK.md`](DEPUTYTASK.md) **CND-9 CHIUSO** (questa PR). **CND-7** CHIUSO (#574). **CND-6** CHIUSO (#575). **CND-3** CHIUSO (#571).
> **Mappa**: CND-0…CND-4/6/7/9/11 su `main` (CND-9 in merge). **Ora**: CND-W (Word flag). Prossimo dopo CND-9: CND-8. Stream STUD-1 parallelo (WPQR).
...
| **CND-9** | Rete di salvataggio officina: `useNdtAutoSave` → IndexedDB `syncQueue` (tipi NDT **già** in `syncService`) | `useNdtAutoSave.js`, gancio save in `NdtReportsPage` | — | ✅ 26/08 | [`DEPUTYTASK.md`](DEPUTYTASK.md) CHIUSO; residuo foto offline (CND-6 messaggio) |
...
| Esecuzione: rete assente | Coda NDT collegata (CND-9); foto allegati offline = residuo | CND-9 *chiusa*; foto = backlog |
