...
| **CND-8** | Crea verbale come audit: bozza UUID → form → coda sync (niente nuova entità “incarico”) | `NdtReportsPage` lista + `enqueueNdtReportSync` / create | CND-9 | ✅ 26/08 | [`DEPUTYTASK.md`](DEPUTYTASK.md) CHIUSO; `seedNdtLocalDraft` + lista onesta; **hotfix tenant** `organization_id` su bozze localStorage (leak Mason↔Camellini) |
