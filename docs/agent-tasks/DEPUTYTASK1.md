...
6. Residuo esplicito: parametri UT sul verbale = CND-5 restante (HITL modello). ✅ dichiarato

## File previsti

- `app/src/pages/EquipmentPage.jsx` / `.css` (solo etichette/ruoli)
- `app/src/utils/ndtInstrumentRoles.js` (costante condivisibile; verbale NDT non toccato in questa slice)
- `app/src/tests/equipmentPage.cnd5a.roles.test.jsx`
...
- [x] Ruoli non-VT selezionabili in anagrafica strumenti
- [x] Nessun tocco verbale UT
- [x] Build OK (+ test se presenti)
- [x] Brief CHIUSO — TEST OK; residuo CND-5 UT dichiarato

## Esito

- Select **Ruolo strumento** su `asset_subcategory` (campo DB già esistente): Calibro / Luxmetro / Lampada / **Giogo** / **Sonda** / **Kit PT** / Altro.
- Lista: badge etichetta sotto il nome.
- **Residuo CND-5**: parametri UT sul verbale + eventuale allineamento `INSTRUMENT_ROLE_OPTIONS` in `NdtReportsPage` → dopo modello Mason UT (HITL).
