...
| **IA-11** | Posa norme in **NORME E LEGGI** se l’albero azienda c’è già ✅ | screening `norma` / hint `norma` → `parent_id` sulla cartella `2.3` di quella `company_id` (`resolveFolderByCode`); se l’albero manca → coda «Cartella mancante». Niente duplicati. **Non** init albero, **non** ingest, **non** unificare wizard. [#523](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/523) | IA-5 | AFK |
| **IA-12** | Ingest famiglia: **Carica norme (batch)** sui file già in 2.3 — **in corso** | `normIngest` / `IngestReviewDialog` dai PDF già in registry (cartella 2.3), senza re-upload dal PC; **non** da Modifica documento | IA-11 | AFK |
