...
| **Smoke test Fatturazione piattaforma → catena di 4 bug reali scoperti in cascata (09/08/2026)** — richiesto uno smoke test della pagina "Fatturazione piattaforma"; l'esplorazione (non solo lettura UI, anche log VPS + query dirette) ha scoperto: (1) timeout client fisso (15s prod) troppo corto per `runReprocessTask` (~15s/candidato, sequenziale) → falso "Richiesta timeout" mentre il backend completava correttamente pochi secondi dopo; (2) `ingestStaging.controller.js` usava `fs.createReadStream` **senza mai importare `fs`** (bug da giugno 2026, mai coperto da test) → anteprima documento sempre in errore 500, per qualunque doc_type; (3) superadmin **non può vedere** la coda di revisione di un tenant cliente diverso dal proprio (scope `organization_id` sempre applicato, anche a superadmin) — non un bug, ma un vincolo di isolamento dati da spiegare chiaramente al committente prima di proseguire. | **Un fix per bug, mai un fix-omnibus**: 3 PR separate (#363 timeout, #364 anteprima, #365 UI coda) anche se scoperte nella stessa sessione — ognuna testabile/revisionabile in isolamento. **Diagnosi via log prima di dichiarare "funziona/non funziona"**: il log VPS (`journalctl -u sgq-backend`, formato Winston `printf` **non include i campi meta extra** passati a `logger.error(msg, {meta})` — solo `message` — quindi un errore reale può risultare invisibile in log finché non si testa la funzione direttamente con uno script Node sul VPS). **Verifica end-to-end reale prima del fix**: lanciata la rielaborazione vera in produzione (non solo letta la UI) per confermare la causa radice (`~60s` per 4 candidati vs timeout 15s) — evitato un fix "a naso". | PR #363, #364, #365 |
| **Coda rielaborazione: da sola-lettura a editabile+raggruppata+difensiva (09/08/2026, 3 iterazioni sullo stesso componente)** — su richiesta del committente, `ReprocessQueueBanner.jsx`/`ReprocessProposalDialog` è passato da (a) dialog fisso piccolo a (b) "Ingrandisci affiancato" riusando `useIngestReviewSplit`/classi `IngestReviewDialog.css` a (c) valore editabile (`FieldInput` esportato da `IngestReviewDialog.jsx`, riusato non duplicato) + raggruppamento client-side per documento (`target_qualification_id`/`target_wpqr_id`, nessuna migrazione). Il gate Bugbot su quest'ultima slice ha trovato **4 problemi reali**, il più grave: confermare un campo **svuotato** marcava comunque la proposta `confirmed` e la rimuoveva dalla coda, mentre il backend (`applyFieldReprocessUpdate`) ignora silenziosamente i valori vuoti — nessuna scrittura, nessun errore visibile, proposta persa. | **Un campo "solo conferma" di una proposta AI è un caso diverso da un form con default/fallback**: se l'operatore lo svuota, la conferma **deve essere bloccata** (non "confermare comunque con valore vuoto"), perché non c'è un secondo campo che salva la situazione come nel form di caricamento completo. **Riuso vero = stesso contratto, non solo stesso componente**: riusare `FieldInput` senza applicare anche la stessa coercizione di tipo (`IngestReviewDialog.handleConfirm`: number→`Number()`, boolean→confronto esplicito) lascia il bug "stringa non normalizzata al server" — il riuso di un componente di input va sempre accompagnato dal riuso della logica di submit che lo contorna. **Alias di registro con prefisso** (`wpqr_thickness_max_unlimited` → colonna reale `thickness_max_unlimited`): centralizzare la risoluzione schema in un'unica funzione (`resolveSchemaField`) usata da label, valore leggibile E tipo di input — tre punti diversi che provano solo la chiave esatta degenerano silenziosamente in comportamenti diversi (etichetta grezza ma tipo corretto, o viceversa). | PR #365 (3 commit sulla stessa PR: feature, fix Bugbot) · gate [Bugbot](bc-c8b3e79d-b7c3-55db-97ce-33d8e59e91e2) |

### Testing (Vitest/Jest)

| Lezione | Regola da applicare | Dettaglio |
|---------|--------------------|-----------|
| **Vitest `mockReset: true` azzera i `.mockResolvedValue()` impostati nella factory `vi.mock()`** — un test falliva con `TypeError: Cannot read properties of undefined (reading 'then')` perché `apiService.getCompanies?.()` tornava `undefined` dentro il componente, pur risultando una funzione mock valida se ispezionata nel test. Causa: `app/vitest.config.mjs` ha `mockReset: true` (+ `restoreMocks`/`clearMocks`), che **resetta l'implementazione di ogni mock prima di ogni test** — anche quella definita una sola volta nella factory di `vi.mock(...)`. | Con `mockReset: true` attivo (verificare sempre `vitest.config.mjs` prima di sospettare bug nel componente), impostare `mockResolvedValue`/`mockReturnValue` **dentro `beforeEach`**, non nella factory `vi.mock()`. Pattern già usato in `managementReviews.test.jsx`; ora anche in `qualificationForm14732.test.jsx`. | sessione 22/07/2026 — PR #277 |
