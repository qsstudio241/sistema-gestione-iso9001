...
| **Idoneità visiva unica + gap NDT (03/08/2026)** | Acuità e Ishihara = **un** certificato oculistico nel form (`Certificato idoneità visiva (acuità + Ishihara)`); alias legacy ancora filtrati. Controllo incrociato: `GET /qualifications/vision-fitness-gaps` + banner tab NDT/Salute mansione se manca/scaduta. Tab Salute mansione: filtro `IN` tipi occupational (prima `LIKE '%salute_mansione%'` non trovava nulla). Blocco hard riesame = slice successiva. | branch `cursor/idoneita-visiva-unificata-2431` |
