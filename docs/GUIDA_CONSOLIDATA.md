...
| **Ingest rischi M03 = detect → conferma → insert (15/08/2026)** — stesso ADR-013 del profilo azienda: Excel in anteprima, poi insert. G=4 non entra (scala 1–3). SWOT/FMEA si mappano a mano. Non sovrascrive righe esistenti. Residuo P/G: o entrambi validi 1–3, o nessuno (un solo fattore fuori scala/mancante si azzera, non si persiste a metà). | Copiare `excelRisksM03Detector` + `RiskM03ImportDialog` (classi `did-*`), non inventare un terzo dialog. | `RisksPage.jsx` · ROO-6 · PR #429 |
...
| **Brief APERTO deve essere su `origin/main` prima del deputy (16/08/2026, SB-1 / PR #440)** — il deputy ha letto `DEPUTYTASK2.md` su `main` (ancora brief licenze **CHIUSO**) e ha chiuso con **FIX NON APPLICABILI**. Il brief SB-1 esisteva solo sul branch della PR. Stesso errore del 07/07/2026. | **Prima di «lancia il deputy»**: `git fetch` + `git show origin/main:docs/agent-tasks/<file>` → titolo giusto e `Stato: APERTO`. Se il brief è solo sul branch feature, il deputy non lo vede. Non aprire SB-2 finché SB-1 non è su `main` (ora #440 è mergiata). | [PR #440](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/440) · `sgq-operating-memory.mdc` § Convenzione DEPUTYTASK |
