...
| **Tab Albero Documenti apre la Home (20/08/2026)** — click su Albero (o Catalogo) scrive `?tab=tree` nell'URL. Il router interno (`RouterContext`) metteva **tutta** la stringa nel path da confrontare con le rotte. `/documents?tab=tree` non è `/documents` né un sotto-path `/documents/…` → cade sulla Home (`/`). Stesso effetto su NC `?select=`. Priorità restava ok perché l'URL di default è `/documents` senza query. Deep link con `goto` URL funzionava: al primo carico `getPath()` legge solo il pathname. | **Path del router = solo pathname.** Query e hash restano in `window.location`, non nello stato `path`. Ogni `navigate`/`replace` con `?…` deve avere un test L1: `resolveMatchingRoute('/pagina?x=1', rotte)` resta `/pagina`, non `/`. **Smoke:** cliccare il tab/pulsante, non solo aprire l'URL già pronto — il `goto` maschera il bug. | PR [#496](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/496) · `RouterContext.jsx` · `routerContext.match.test.js` |
