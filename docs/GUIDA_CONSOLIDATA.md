...
| **Lavori paralleli deputy + merge Rischi/ActionPlan (22/07/2026)** — DEPUTYTASK2 (Action Plan P1: picker reclamo + statistiche categoria) e DEPUTYTASK1 (Rischi §6.1 Slices 1-3: campo nature, §4.1/§4.2, link Piano Azioni) completati e mergiati. | **Lezione conflitto rebase**: quando due deputy toccano lo stesso controller (`nc.controller.js`) in parallelo, il rebase produce conflitto sui destructuring `req.body` — tenere ENTRAMBI i campi nuovi. **Lezione deploy deputy**: verificare sempre che il deputy abbia deployato il backend su VPS; se mancante, deployare con `scp + systemctl restart` prima del merge. | PR #278 (Action Plan P1) · PR #279 (Rischi §6.1) |
