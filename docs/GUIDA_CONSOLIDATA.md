...
| **DDT ≠ mill: classifica il PDF prima di copiare la colata (19/08/2026, MC-I3)** — Estrai sul DDT `000775RE` ha trattato la bolla come 3.1 (`heat=692976`, norma merce, `ddt_no` vuoto). | Filename `D.D.T.`/`bolla` → `document_kind=delivery_note` **anche se l’AI dice mill**; `CERTIFICATO`/`3.1` nel nome resta mill. Sanitize heat/cert/norma a NULL e in SQL **SET** (non solo `COALESCE`, altrimenti la colata sbagliata resta al re-extract). Valuta 409 `NOT_A_CERTIFICATE`; pulsante Valuta **visibile**, `disabled` + title. Un PDF → una riga (split = MC-I4). Dopo merge: deploy backend e rieditare Estrai su **id 7**. | brief [`DEPUTYTASK_MC_INGEST.md`](agent-tasks/DEPUTYTASK_MC_INGEST.md) |
