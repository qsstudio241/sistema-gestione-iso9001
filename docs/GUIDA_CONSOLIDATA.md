...
| **Una review Bugbot per PR (18/08/2026)** | Ogni run Bugbot costa ~$1.00–$1.50. Push ripetuti + subagente `bugbot` oltre al check GitHub **moltiplicano** il costo. **1 run fatturabile per PR**; secondo run solo dopo un fix reale. Attendere `Cursor Bugbot` su GitHub; non lanciare anche il subagente / `bugbot run` se il check è già partito. | [`sgq-workflow-method.mdc` § 4](../.cursor/rules/sgq-workflow-method.mdc) · [`sgq-git-autonomy.mdc`](../.cursor/rules/sgq-git-autonomy.mdc) |
