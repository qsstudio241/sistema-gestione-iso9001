...
- **MC-4 (17/08/2026)**: API `/api/v1/material-certificates` — lista/dettaglio/upload, extract (riuso ingest, OCR assente → `ocr_skipped`), evaluate persiste checks e passa a `pending_review`. `compliant` solo da POST approve. Licenza AND `saldatura`+`ai_import`. Nessuna migrazione nuova.
