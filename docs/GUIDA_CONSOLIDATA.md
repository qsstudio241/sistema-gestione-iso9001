...
| **Claim visivo senza aprire la pagina (14/08/2026)** — l'agente ha scritto «la barra è solo Ambito + tendina» senza login. In produzione il banner era ancora logo + Al.project + P.IVA (PR #414 non mergiata). Sulla prima preview la barra era **vuota**: `CompanyScopeSelect` tornava `null` se `companies.length === 0` (caricamento API). I test L1 mockavano già le aziende, quindi erano verdi e ciechi. | **Non dichiarare un layout** senza aprire la pagina (Playwright autenticato o test che monta `AppLayout`). Un test che mocka i dati «felici» non intercetta `return null` / lista vuota. **Non dire «è fatto» su produzione** se la PR è ancora aperta. Il menu Ambito deve restare visibile anche con lista aziende vuota. | **PR #414** · test `appLayoutBanner.test.jsx`, `companyScopeSelect.test.jsx` |
