...
| **Lock `updated_at` DATETIME2 vs Date JS — 409 spurio (18/08/2026, MC-I0)** — Valuta su certificato `extracted` falliva 409 anche se lo stato era ammesso. Causa: `AND updated_at = @updated_at` con `DATETIME2` (100 ns) contro il `Date` di node-mssql (ms). | **Non** usare il timestamp JS del driver come lock ottimista su `DATETIME2`. Race = `workflow_status` (o altro campo di dominio) nella stessa transazione. `compliant` resta 409. | PR [#463](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/463) · smoke PROD azienda 179 |
