...
|---------|--------------------|-----------|
| **Ingest rischi M03 = detect → conferma → insert (15/08/2026)** — stesso ADR-013 del profilo azienda: Excel in anteprima, poi insert. G=4 non entra (scala 1–3). SWOT/FMEA si rifiutano. Non sovrascrive righe esistenti. | Copiare `excelRisksM03Detector` + `RiskM03ImportDialog` (classi `did-*`), non inventare un terzo dialog. | `RisksPage.jsx` · ROO-6 |
