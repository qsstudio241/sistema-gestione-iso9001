...
| **Import PDF batch (Sprint 9)** | Tabelle `import_jobs`, `import_job_files`; API `GET/POST/PATCH/DELETE /import-jobs`, upload `POST .../files` (multipart `files` + `relative_paths`), `POST .../process` usa `pdf-parse` + `confidenceFromTextLength` (euristica). **`POST .../files/:fileId/ai-extract`**: estrazione JSON strutturata via OpenAI sul testo già estratto (richiede `OPENAI_API_KEY` sul server; rate limit dedicato). Colonne file: `ai_extraction_json`, `ai_extraction_error`, `ai_extraction_at`, `ai_model` (migrazione **039**). Licenza modulo **`ai_import`**. UI admin: **Impostazioni → Import PDF** (`/settings/import-jobs`). **IA-4/IA-5 (20/08, #506/#509)**: «Carica cartella» (`webkitdirectory`); path relativo in `original_name` (niente colonna nuova, max 80 PDF); **Screening e posa** (`POST .../screen-and-place`) classifica da path+nome+testo e posa in scaffale solo se confidence alta + azienda + tipo non-qualifica. Senza azienda: `400 COMPANY_REQUIRED_FOR_FOLDER`. Deploy VPS: manifest include `importRelativePath.js`, `importScreening.js`, `importJobs.controller.js` + routes + restart. **Privacy**: il testo inviato all’API è lo stesso mostrato in schermata revisione; valutare accordo/DPA OpenAI per l’organizzazione. |
