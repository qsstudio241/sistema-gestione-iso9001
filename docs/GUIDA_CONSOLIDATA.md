...
| **Più chat in parallelo + 1 Bugbot (18/08/2026)** | Sì al multitasking: ogni chat un modulo, file disgiunti; file hub (GUIDA, roadmap, `PROJECT_CONTEXT`, `DEPUTYTASK.md`, manifest) = un solo owner. Desktop = worktree, non due finestre sulla stessa cartella. **Costo:** Bugbot always-on addebita ogni push; in Dashboard impostare **Run only when mentioned** + un `bugbot run` a lavoro finito. | [`sgq-workflow-method.mdc` § 0 e § 4](../.cursor/rules/sgq-workflow-method.mdc) · `sgq-operating-memory.mdc` § Parallelo |
