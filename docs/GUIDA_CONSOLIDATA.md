...
| **Anagrafica tenant (P.IVA + logo)** | Colonne `organizations.vat_number`, `organizations.logo_url` (migrazione **041**). API: `GET/PATCH /organizations/me`, upload `POST /organizations/me/logo`, `DELETE …/logo` — vedi [BACKEND_API.md](BACKEND_API.md). Dopo provisioning dell’account sulla nuova org, l’**admin** compila da **Impostazioni → Organizzazione** (`/settings/organization`); stesso form anche in **Licenze moduli** per comodità. Deploy VPS: `run-migration-041.js` se mancano colonne + copia `organization.controller.js` / `organization.routes.js` + restart. |
