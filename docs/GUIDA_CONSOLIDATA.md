...
- **Upload template**: copiare `.docx` in `public/templates/` **non** basta — registrare con **POST** `/api/v1/report-templates` e assegnazione checklist/org. **Da UI**: **Gestione → Template report** — banner **solo Carica file**; **Duplica** solo sulla riga elenco (modal nome). Non un secondo Duplica nel banner. Duplica gira sul VPS e legge `backend/templates/` (copia dei modelli di sistema): i `.docx` in `app/public/templates` stanno su Netlify, il backend non li vede — senza quella copia → `SOURCE_NOT_FOUND`. Upload **senza** obbligo `standard_key` ISO. Assegnazione ISO sotto griglia; checklist custom in **Checklist personalizzate**. Warning soft se mancano `CHECKLIST_MARKER` / `RILIEVI_MARKER`. API: `POST /report-templates/:id/duplicate` `{ name }`.
