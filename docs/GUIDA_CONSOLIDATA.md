...
| **Pattern modal permanente + auto-save (13/09/2026)** — le finestre di editing non si chiudono cliccando fuori e salvano automaticamente senza pulsante Salva esplicito. | Overlay senza `onClick={onClose}` (chiude solo con X). `useEffect` su `form` state con debounce 800ms → chiama `apiService.updateXxx` se ID presente, altrimenti `apiService.createXxx` (salva ID restituito in `savedIdRef`). Refs: `savedIdRef`, `isFirstRender` (skip primo render), `autoSaveTimer`. Badge stato header: `.autosave-status.saving/saved/error`. Footer: solo pulsante **Chiudi** (disabilitato durante save). Prop `onSaved` per notificare il parent di ricaricare la lista. **Allegati (upload file): NON inclusi nell'auto-save** — richiedono pulsante Allega esplicito separato. Modello: `RiskForm` in `RisksPage.jsx`. | Applicato a: RiskForm, QualificationForm (`qf-overlay`), SupplierFormModal, CompanyFormModal, ComplaintFormModal, EquipmentFormModal, ReviewFormModal, ProjectFormModal, WPSFormModal, WPQRFormModal, CounterpartyFormModal |
