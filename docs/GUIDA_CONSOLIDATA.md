...
| **ADR-017 — rete ingest + catalogo UNI (luglio 2026)** | Tre livelli: A regole globali, B `ingest_reference_patterns` (pattern anonimi cross-tenant, no PII), C few-shot org. Catalogo **UNI Store primario** (`uniStoreConnector`) prima di ISO.org. Prompt AI: `buildIngestLearningPromptSection`. Prossimo: IG-N norme in staging (`DEPUTYTASK.md`). Mig. **120** su VPS **OK** (04/07/2026). | [ADR-017](adr/ADR-017-ingest-reference-network.md) |
| **UNI Store — tre livelli di accesso, non confondere** | (1) **Lookup catalogo/vigore** (`uniStoreConnector.service.js`) usa le **API pubbliche** (nessun login) → già funzionante, verificato su `UNI EN ISO 3834-2:2021`. (2) **Login completamente automatico** (senza intervento umano) → **bloccato** da anti-bot UNI (nessuna risposta HTTP alla chiamata di auth). (3) **Login "assistito"** (browser visibile, credenziali precompilate, click di login eseguito da una persona reale) → **FUNZIONA** (testato 05/07/2026, JWT ricevuto), e la sessione si può salvare (`context.storageState`) per non richiedere il login ad ogni utilizzo. Dopo login assistito, la navigazione automatica alla scheda norma **funziona** e mostra correttamente titolo/dati aggiornati. **Limite residuo**: se la norma non è inclusa nell'abbonamento ma è acquistabile singolarmente, la scheda mostra un flusso di **acquisto** ("Scarica" con quantità/formato) — **mai cliccare in automatico**, rischio di addebito reale. Anche a download avvenuto, il PDF resta protetto da **DRM FileOpen** (dichiarato da UNI in ogni pagina): illeggibile da script (`pdf-parse` ecc.) senza il visualizzatore proprietario attivato dall'utente. **Da testare**: se una norma è già inclusa nell'abbonamento attivo (non acquisto singolo), la modalità "consultazione integrale" potrebbe usare un lettore diverso — verificare se espone testo leggibile via DOM prima di escludere definitivamente l'estrazione assistita. | [ADR-010](adr/ADR-010-ai-agentic-architecture.md#stato-implementazione-2026-06-hk-harness-hardening) · script diagnostico `backend/scripts/uni-store-diagnostic.js` (env `UNI_STORE_HUMAN_LOGIN=true`) |
