...
4. **Esperienza 22/05/2026 (Camellini)**: osservazione corretta — in app compariva ancora la matrice **legislativa** (VIA, AIA, rifiuti…) perché `app/src/data/checklistTemplates.js` non era allineato al DB post-049. Fix: `ISO_14001_TEMPLATE` = audit SGA (53 domande); matrice legislativa spostata in `ISO_14001_LEGISLATIVO_TEMPLATE` (futura checklist custom). `initializeChecklist` re-inizializza audit con sezioni `14001_s4/s5` obsolete. Rigenerazione template: `node app/scripts/buildIso14001Template.js` + export JSON da VPS.
