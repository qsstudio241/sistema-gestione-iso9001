...
|---------|--------------------|-----------|
| **Chrome unico viewer documenti (04/09/2026)** — PDF/Word/Excel devono sembrare la stessa finestra in-app. | Nuovo visualizzatore = `DocumentViewerChrome` (Chiudi + Scarica se previsto + Schermo intero/Riduci). Overlay sul viewport, non Fullscreen API e non finestra Windows. Vietato imitare ─ □ ✕. `DocFileDialog` resta il form file. | `DocumentViewerChrome.jsx` · [LIBRERIA](reference/LIBRERIA_UI_SGQ.md) · regola `.cursor/rules/sgq-document-viewer-chrome.mdc` |
| **Libreria ≠ Documenti / Knowledge Health (29/08/2026, LN-1)** — qualità fonti AI (norme+libri/riferimenti) non è l’inventario operativo Documenti né i KPI chunk. SoT resta `document_registry`; vigore solo su `norma`; non-norma = `issue_date` se c’è; richieste = backlog MD/JSON read-only (HITL PDF fuori UI). | Voce **Gestione → Libreria** (`/settings/libreria`). Niente secondo inventario, niente `doc_type` libro/quaderno senza gate, niente colonna `publication_date` in LN-1. | [`PLAN_LIBRERIA_NORME_SLICES.md`](agent-tasks/PLAN_LIBRERIA_NORME_SLICES.md) · `NormLibraryPage.jsx` |
| **Layout mobile: scroll, non `100vh` nello shell (09/09/2026, #673)** — header + Ambito + footer copyright + bottom nav schiacciavano le tab (Assistente: input sopra la barra). Causa condivisa, non una pagina sola. | Su ≤768px: nascondere `.layout-footer`; padding-bottom = bottom nav + safe-area; **niente** `height: calc(100vh)` dentro `layout-main`. Linguette = una riga + `overflow-x: auto` (non `flex-wrap: wrap`). **Ordine CSS:** la media query mobile deve stare **dopo** la regola desktop, altrimenti `wrap` vince. Test L1 `mobileLayoutChrome.test.js`. Refresh PWA dopo deploy. | PR [#673](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/673) · `AppLayout.css` · `AiAssistantPage.css` · `QualificationsPage.css` |
| **Hamburger mobile per voci solo-sidebar (29/08/2026, #604)** — su viewport ≤768px la sidebar era `display:none` e la bottom nav non esponeva Gestione: Libreria (e altre settings) irraggiungibili dal telefono. | Nuova voce solo in sidebar → **hamburger + drawer** che riusa la stessa nav (`mobileNavOpen`); `NavLink` chiude il drawer al click. Test L1 `appLayoutMobileNav.test.jsx`. Non duplicare le voci settings nella bottom nav. | PR [#604](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/604) · `AppLayout.jsx` / `.css` |
...
| **NC ↔ commessa è un ponte opzionale (ISO-6, 18/08/2026)** — ISO 3834-3 §15 non obbliga ogni NC a una commessa. Stesso pattern Welding Book: picker visibile, `disabled` senza azienda. | `project_id` **NULL ammesso**. Validare org + azienda se presente; FK `ON DELETE SET NULL`, mai CASCADE. Non mescolare con ingest MC. | Migrazione **153** · `nc.controller.js` · `NcCreateModal.jsx` |
| **RDP/NDT ↔ commessa è un ponte opzionale (ISO-7, 19/08/2026)** — stesso pattern NC. Il testo libero `project_name` / `job_order` resta. | Helper `resolveOptionalProjectId`. In **update**, se cambia `company_id` e `project_id` è omesso, rivalidare (400 `PROJECT_COMPANY_MISMATCH`) — non lasciare la commessa della vecchia azienda. Migrazione **155** prima su TEST. | PR [#474](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/474) · `rdp.controller.js` · `ndtReports.controller.js` |
| **RDP Mason = verbale di visita, non laboratorio (19/08/2026)** — Mason usa `RDP_MSN-AAMMGG-NN` come codice visita. Check list 27/01 = Audit ISO 3834-2 (id 6). Menu Saldatura→RDP spento (zero record vivi). Standard Audit id 7: solo rinomina UI, codice `RDP_MSN` invariato. | Non copiare i Word Mason su `RDPModule`. Non droppare `rdp_*`. Word ISO-4 dal modulo **Audit**. Laboratorio vero = CND. Scala 1–6 = ISO-4b (HITL). | [gap 19/08](gap-reports/GAP_RDP_DUE_DOCUMENTI_MASON_2026-08-19.md) · [PLAN](agent-tasks/PLAN_3834_SLICES.md) · PR [#486](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/486) |
| **ISO-4 Word visita = Quesito/Evidenze/Esito (07/09/2026)** — l’export Audit 3834 riusava le colonne ISO 9001 e anteponeva «Rilievi pendenti». Mason 27/01 è visita a quesiti. | Iniezione checklist `ISO_3834_2`/`RDP_MSN`: intestazioni **Quesito \| Evidenze (eventuali foto) \| Esito** (`STATUS_CFG` C/NC); niente cap. Rilievi pendenti. Scala 1–6 resta ISO-4b. | [`PLAN_3834`](agent-tasks/PLAN_3834_SLICES.md) · `wordExportHelpers.js` · PR [#668](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/668) |
| **ISO-5 Word Welding Book = IOF programmatico (08/09/2026)** — WB senza export; pattern WPS Annex A già collaudato. | Nuovo `wordExportWeldingBook.js` (testata + attrezzature + sequenza + parametri); niente esiti C/NC; foto cordone persistite = ISO-5b. | [`PLAN_3834`](agent-tasks/PLAN_3834_SLICES.md) · `WeldingBooksPage.jsx` · branch `cursor/iso5-word-welding-book-8269` |
| **ISO-5b foto cordone — upsert welds, non DELETE orfani (08/09/2026)** — allegati su `attachments.welding_book_weld_id` (mig. **165** + CHK parent). Un `DELETE`+reinsert delle righe saldatura a ogni salvataggio spezzerebbe gli id e lascerebbe foto orfane. | `replaceWelds` = **upsert-by-id** (pattern RDP): aggiorna/inserisce, cancella solo id non più presenti e senza allegati bloccanti. UI: salva prima → abilita upload riga; Word = `ImageRun`. | PR [#671](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/671) · `weldingBooks.controller.js` · `WbWeldAttachments.jsx` |
...
| **Query string ≠ pagina — un solo navigatore (20/08/2026)** — Albero Documenti (`?tab=tree`) apriva la Home: `RouterContext` confrontava l'URL intero con le rotte. Stesso schema ovunque si naviga con `?`: Catalogo, NC `?select=`, Qualifiche da dashboard/scadenzari, SAL da Gap Analysis, Reclami da scheda NC, citazioni/ricerca verso Documenti. Il `goto` dell'URL maschera il bug (al primo carico si legge solo il pathname). | **Regola:** pagina = pathname; `?` e `#` = dettaglio (tab, riga, filtro). Strip **solo** in `RouterContext` (`pathnameOnly`), non copiato nei moduli. Nuovo `navigate('/x?y=1')` → riga nel test L1 `resolveMatchingRoute` (resta `/x`, mai `/`). Smoke: click sul controllo. | PR [#496](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/496) · `RouterContext.jsx` · `routerContext.match.test.js` |
...
| **CND-2 gate ispettore 9712 + visione (25/08/2026)** — il nome libero sul verbale non basta per Completa/firma. Serve patentino ISO 9712 **valido per il metodo** (livello 2 o 3, estratto §5.3.2) **e** idoneità visiva. | Completa e A/R/S restano visibili (`disabled`+`title` se il gate fallisce). Bozza sempre ok. Errore rete: A/R/S usabili in bozza, Completa bloccato, retry su `online`. In modifica il **metodo** è immutabile (numero verbale): PUT e UI usano quello in DB. `GET /ndt-reports/inspector-eligibility` **prima** di `/:id` + `company_access`. 409 `NDT_INSPECTOR_GATE`. Niente migrazione (match sul nome). Dopo merge: deploy VPS (`ndtInspectorGate.service.js` nel manifest) altrimenti eligibility 404. | PR [#561](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/561) · `ndtInspectorGate.service.js` · `NdtReportsPage.jsx` |
| **CND-3 flag PT/MT nel JSON (26/08/2026)** — Mason: checkbox Word PT/MT sul verbale. Un verbale = un metodo (`report_type`): mai mix PT↔MT. JSON: `method_params.pt` / `.mt` (catalogo Mason); VT resta piatto (lux). Placeholder `{pt_acc_l2}` = semantici, non FORMCHECKBOX. Nomi/date in testata. Export Word = slice dopo CND-4. | `sanitizeMethodParams` al save. Cambio `report_type` → `defaultMethodParams`. UI: `status-btn` come NC. Accenti: `\u` in stringa JS, non JSX letterale (già riga sotto). | PR [#571](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/571) · `ndtMethodParams.js` · `NdtReportsPage.jsx` |
| **NG-4 norma assente in chat/gap (26/08/2026)** — NormBroker già tornava `null`; chat/gap potevano comunque «inventare» il testo. | Messaggio stabile in italiano: testo non in archivio → non valutare a caso → Registro / Carica norme. Graceful (chat non spegne). Riuso pattern SAL `textAvailable=false`. | PR [#570](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/570) · `normBroker.service.js` · `aiChat` / gap / context builder |
| **CND-7 posa verbale nel Registro (26/08/2026)** — il verbale restava solo in `ndt_reports`, fuori fascicolo SGQ. | Su Completa/Approva: posa `report_ndt` in 9.3 (`ndtReportRegistryPose`), idempotente via `type_specific_data.ndt_report_id`; cartella assente → messaggio, non crea albero. Errore posa non blocca il salvataggio verbale. Deploy VPS (manifest). | PR [#574](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/574) |
| **CND-6 foto + NC da marca (26/08/2026)** — in officina target piccoli e NC da R/S poco chiari. | Touch/camera su `NdtItemAttachments`; hint → `NcCreateModal` da giudizio R/S. Niente secondo uploader. | PR [#575](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/575) |
| **CND-9 rete salvataggio officina (26/08/2026)** — bozza solo localStorage; tipi NDT in coda già previsti ma non agganciati. | Offline su Salva/Completa → `enqueueNdtReportSync` / `syncService`; banner senza rete; clear draft a sync. Harden post-Bugbot #579. **Residuo:** foto allegati offline (niente blob in coda). | PR [#578](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/578) · [#579](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/579) |
| **CND-W export Word PT/MT (26/08/2026)** — flag in JSON ma Word senza checkbox. | `buildPtMtPlaceholderData` → placeholder semantici PLAN; resolve template CND-4 invariato; VT lux non regredisce. | PR [#577](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/577) · `vtWordExport.js` |
| **CND-8 bozza verbale come audit (26/08/2026)** — crea da zero senza UUID subito. | «Nuovo verbale» → `seedNdtLocalDraft` (UUID); lista bozze onesta; offline → coda CND-9. Niente entità incarico. | PR [#582](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/582) |
| **localStorage / coda offline = sempre scope tenant (26/08/2026)** — CND-8 salvava bozze `sgq:ndt_draft:*` **senza** `organization_id`; la lista server era filtrata per studio, il merge locale no → Mason vedeva «Bozza locale» con ispettore di un altro studio (stesso browser, altro login). Errore di design della slice, non del DB. | **Regola**: ogni bozza/coda offline in `localStorage` o IndexedDB va **keyed o filtrata** per `organization_id` (e `user` se serve). Merge lista server+locale = **stesso scope tenant del GET**. Bozze legacy senza org_id: **non mostrare** se c'è org loggata (niente leak). Gate: `ndtDraftsTenantScope.test.js`. | hotfix `useNdtAutoSave` / `NdtReportsPage` · test `ndtDraftsTenantScope.test.js` |
| **CND-5a ruoli strumento non-VT (26/08/2026)** — anagrafica solo gauge/lux/lamp. | Ruoli giogo / sonda / kit PT (`ndtInstrumentRoles.js` + Equipment). UT sul verbale = HITL (manca modello). | PR [#581](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/581) |
...
| **WPQR t1/t2 duali + 15614-2 alluminio (25/08/2026, Mason)** — i verbali FW con due spessori diversi venivano collassati in un solo `thickness_min/max` (dato perso + copertura WPS sbagliata). In più si confondeva **9606-2** (patentino alluminio, già in Qualifiche) con la norma di procedura WPQR (**15614-2**). PDF consegnati → digitalizzati `NORMA_00031`/`00032`. In corsa: commento script migrazione con testo `t1_*/t2_*` → `*/` chiudeva il blocco `/*` e Node sul VPS crashava; flag `t1`/`t2` `_max_unlimited` andavano nel round-trip sentinel come il flag legacy. | **Due range DB** (`thickness_t1_*` / `thickness_t2_*`, mig. **158**), ingest che li risolve, copertura WPS A↔t1/B↔t2 o scambiati, select 15614-1/15614-2, regole Tabella 5/6/7 in `weldingQualificationRules15614_2.js`. **Mai** `*/` dentro commenti blocco JS. Nuovi `*_max_unlimited` → forzare `false` nel sentinel. **9606-2 ≠ WPQR.** Codificare solo ciò che è leggibile dal Markdown norma. | PR [#558](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/558) · [brief](agent-tasks/DEPUTYTASK_WPQR_T1T2.md) · [gap](gap-reports/GAP_WPQR_T1T2_ALLUMINIO_2026-08-25.md) · [estratto 15614-2](reference/ISO-15614-2-range-validita-WPQR.md) |
| **WPQR Stud / prigioniero + P+T (26/08/2026, Mason)** — dopo t1/t2 restava il gap verbale 001P-21: niente tipologia stud, niente «elemento che si qualifica», diametro etichettato solo «tubo», un solo materiale, `product_type` solo P\|T. ISO **14555** assente → non si inventano range stud. | **STUD-1 campi** (mig. **159**): joint `SW` dedicato (≠ FW), `qualifying_element`, riuso `diameter_*` con label prigioniero se SW, Parent Metal 2, `P+T` + regola piastra→tubo su P+T senza diametro. Registro rielaborazioni allineato. STUD-3 = dopo PDF 14555. | [brief](agent-tasks/DEPUTYTASK_WPQR_STUD.md) · [gap](gap-reports/GAP_WPQR_STUD_WELDING_PIASTRA_TUBO_2026-08-25.md) · mig. 159 |
| **Rielaborazioni: conteggio candidati vs proposte pending (25/08/2026)** — dopo «Lancia» il pannello teneva lo stesso conteggio alto perché i record restano NULL in tabella finché non si conferma in Qualifiche/Saldatura; le proposte `ingest_staging` pending venivano ancora contate come candidati (doppio lavoro AI). In più l’elenco mostrava voci a 0. | **Candidati** = campo ancora NULL **e** senza proposta pending sullo stesso target/campo. FE: nascondi righe a 0; niente lanci paralleli. Gate registro su nuovo campo ingest: vincolante in `sgq-operating-memory.mdc` (non solo GUIDA). | PR [#566](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/566) · [#567](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/567) · `qualificationReprocess.service.js` · `BillingDashboardPage.jsx` |
...
| **Pattern modal permanente + auto-save (13/09/2026)** — le finestre di editing non si chiudono cliccando fuori e salvano automaticamente senza pulsante Salva esplicito. | Overlay senza `onClick={onClose}` (chiude solo con X). `useEffect` su `form` state con debounce 800ms → chiama `apiService.updateXxx` se ID presente, altrimenti `apiService.createXxx` (salva ID restituito in `savedIdRef`). Refs: `savedIdRef`, `userEditedRef` (skip idratazione, solo edit utente), `autoSaveTimer`. Badge stato header: `.autosave-status.saving/saved/error`. Footer: solo pulsante **Chiudi** (disabilitato durante save). Prop `onSaved` = **solo ricarica lista**, mai chiudere il modal (`onSaved={loadData}`, come ProjectsPage/EquipmentPage). **Allegati (upload file): NON inclusi nell'auto-save** — richiedono pulsante Allega esplicito separato. Modello: `RiskForm` in `RisksPage.jsx`. | Applicato a: RiskForm, QualificationForm (`qf-overlay`), SupplierFormModal, CompanyFormModal, ComplaintFormModal, EquipmentFormModal, ReviewFormModal, ProjectFormModal, WPSFormModal, WPQRFormModal, CounterpartyFormModal |
| **Qualifiche: finestra modifica si richiude da sola (16/09/2026, Mason)** — `onSaved={handleSaved}` chiudeva il modal dopo l'auto-save (~800 ms). In più `isFirstRender` non basta: il form parte da EMPTY e l'idratazione da `qualification` è un secondo `setForm`, trattato come edit. | `onSaved={loadData}`. Auto-save solo dopo edit utente (`userEditedRef`). Chiude solo X / Chiudi. | `QualificationForm.jsx` · `QualificationsPage.jsx` · test `qualificationFormAutosave.test.jsx` |
| **Modal con classi CSS non standard sfuggono alle ricerche (13/09/2026)** — la finestra qualifiche usava `qf-overlay`/`qf-header`/`qf-modal` invece di `modal-overlay`/`modal-header`/`modal-box`: la ricerca per pattern `modal-overlay` l'aveva saltata e la finestra non era stata aggiornata. | Prima di dichiarare «tutte le finestre aggiornate», cercare **varianti di classe** (`*-overlay`, `*-modal`, `*-header`) oltre a `modal-overlay`. Comando utile (PowerShell): `Select-String -Path app\src\**\*.jsx -Pattern 'onClick.*onClose\|onClose.*onClick' -Recurse` — intercetta tutti i punti di chiusura, indipendentemente dalla classe CSS usata. | `QualificationsPage.jsx` — classe `qf-overlay` rinominata a `modal-overlay` nel contesto del pattern auto-save |
| **Header/footer sticky nei modal — `overflow: hidden` blocca `position: sticky` (13/09/2026)** — `position: sticky` su `.modal-header` non funziona se il parent `.modal-box` ha `overflow: hidden`: la testata scorreva insieme al contenuto. | Cambiare `.modal-box` da `overflow: hidden` a `overflow-y: auto` **prima** di applicare sticky. Regola CSS corretta: `.modal-box { overflow-y: auto; max-height: 90vh; }` · `.modal-header { position: sticky; top: 0; z-index: 10; background: #fff; }` · `.modal-footer { position: sticky; bottom: 0; z-index: 10; background: #fff; }` | Pattern trasversale — tutti i modal SGQ con header/footer fissi |
...
| **Conclusioni audit AI — testo editabile prima di Accetta (20/08/2026)** — il modal mostrava la proposta in un `div` sola lettura; il revisore non poteva correggere. | **HITL**: textarea + stato `editedText`; Accetta passa il testo corretto (non l'originale). Feedback: `aiText` = proposta, `finalText` = versione accettata. Non reintrodurre un riquadro non editabile. | PR [#498](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/498) · `AiConclusionsModal.jsx` |
| **SAL S1a — OCR PDF nell'estrattore riusabile (18/08/2026)** — i PDF scansionati non arrivavano al suggeritore SAL: `documentTextExtractor` si fermava a `pdf_no_text_layer` mentre l'ingest aveva già `ocrExtractor`. | **Riusare** `extractTextWithOCR` (stessa soglia `INGEST_OCR_MIN_CHARS`, default 50). Non duplicare lo stack. Fallimento → `ocr_unavailable` / `ocr_failed`, mai throw. UI e `salAiSuggest` non si toccano: se l'OCR produce testo, il messaggio «PDF immagine» non compare. Immagini = slice **S1b** (stesso file, non in parallelo). | [PR #471](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/471) · [PLAN](agent-tasks/PLAN_SAL_AI_EVIDENCE_SLICES.md) |
| **SAL AI evidenze S1b+S2a+S2b (29/08/2026)** — OCR immagini; suggest tipo+candidati a evidenza vuota; UI collega/carica/ignora. | S1b: PNG/JPEG/WebP via Tesseract (`extractTextFromImageBuffer`). S2a: `missingEvidenceSuggestion` sempre presente, **zero write** FK. S2b: blocco solo se oggetto; **Collega** = PATCH `evidenceDocumentIds`; Ignora/Carica = nessuna write stato. Piano chiuso; S1c `.doc` solo su richiesta. | [PLAN](agent-tasks/PLAN_SAL_AI_EVIDENCE_SLICES.md) · [#603](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/603) · [#605](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/605) · `SalAiSuggestDialog.jsx` |
...
| **OCR scan ≠ ocr_skipped (19/08/2026, MC-B)** — Estrai su PDF scansionato etichettava `ocr_skipped` anche quando l’OCR (S1a) c’era. `ocr_unavailable` veniva collassato in skipped. | **`ocr_skipped` solo se il file non è un PDF.** OCR ok → `ocr_ok` sull’estrattore (non `text_layer`). Motore assente → `ocr_unavailable`; errore → `ocr_failed`; HTTP 200, mai 500. Riusare `documentTextExtractor`, non un secondo motore. | PR [#476](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/476) · deploy TEST PID `993561` + PROD PID `1005497` · smoke PROD id 7 `ocr_ok` |
| **Colata 3.1 da alias AI, non solo heat_or_lot_no (19/08/2026, MC-I2)** — Estrai sul 3.1 Tecnovespa non riempiva la colonna colata (`12174/2026`): l’AI scrive `heat_number` / `colata` / `B07`, il mapping leggeva solo `heat_or_lot_no`. `ddt_no` non andava in UPDATE. | **Canonicalizzare gli alias** (anche in `extracted_json`) prima di `applyAnagraficaFromJson`, poi **togliere gli alias** dal JSON. Persistire `ddt_no`/`ddt_date`. **Non** copiare A07 (`purchaser_order_no`) nel DDT. Fallback testo **solo** se etichettato (Colata / Heat No / B07), mai `NNNN/YYYY` isolato. PATCH che svuota il DDT deve restare vuoto (niente re-iniezione da `delivery_note_no`). Dopo merge: deploy backend e rieditare Estrai. | PR [#481](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/481) · deploy TEST PID `1019412` + PROD PID `1031291` · smoke PROD id 6 colata `12174/2026` · brief [`DEPUTYTASK_MC_INGEST.md`](agent-tasks/DEPUTYTASK_MC_INGEST.md) |
| **DDT ≠ mill: classifica il PDF prima di copiare la colata (19/08/2026, MC-I3)** — Estrai sul DDT `000775RE` ha trattato la bolla come 3.1 (`heat=692976`, norma merce, `ddt_no` vuoto). | Filename `D.D.T.`/`bolla` → `document_kind=delivery_note` **anche se l’AI dice mill**; `CERTIFICATO`/`3.1` nel nome resta mill. Sanitize heat/cert/norma a NULL e in SQL **SET** (non solo `COALESCE`, altrimenti la colata sbagliata resta al re-extract). Valuta 409 `NOT_A_CERTIFICATE`; pulsante Valuta **visibile**, `disabled` + title. Un PDF → una riga (split = MC-I4). Dopo merge: deploy backend e rieditare Estrai su **id 7**. | brief [`DEPUTYTASK_MC_INGEST.md`](agent-tasks/DEPUTYTASK_MC_INGEST.md) · PR [#488](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/488) |
...
| **LLM Wiki / Engram ≠ prodotto (06/09/2026)** — video vault Obsidian + fatti esterni al prompt. | **Nessun** vault Obsidian in-app. KB prodotto = DB + moduli esistenti. Principio Engram già in Second Brain (`ambitoFacts` SQL). Dieta token = stringere harness (AGENTS/rules/piano), non nuova architettura. | `PROJECT_CONTEXT.md` § Harness · `PLAN_SECOND_BRAIN_SLICES.md` |
| **Compliance Map ≠ SAL ≠ Second Brain (06/09/2026, CM-1)** — grafo requisito↔fonte↔gap in SQL, non vault. | Mappa scoped org+company, versionata, HITL (`proposed`/`accepted`/`edited`/`rejected`) + `compliance_map_events`. SAL = maturità SGQ. Second Brain = fatti live. Catalogo `norm_requirements` piattaforma; link negli items. | mig. **164** · `complianceMap.service.js` · [`PLAN_COMPLIANCE_MAP_SLICES`](agent-tasks/PLAN_COMPLIANCE_MAP_SLICES.md) |
| **CM-5 citazioni Assistente solo HITL confermati (07/09/2026)** — blocco prompt da mappa `approved`, items accepted/edited only; niente proposed/rejected né NC live come copertura. | Export JSON/MD stessa policy; citazioni `compliance_map_item` → `/compliance-maps?select=&highlight=`. Deploy BE dopo merge. | `complianceMapChat.service.js` · `aiChat.controller.js` |
| **CTX-3 proposte registro = checkbox, mai overwrite (07/09/2026)** — enrich OpenAPI propone `sector`/`source`/`source_url` citati; conflitto con campo già pieno → checkbox off di default. | Applica solo selezionati; **Salva anagrafica** resta obbligatorio. Niente auto-save. OAuth Email/Drive = CTX-4 Alto (HITL, non aprire senza DoD). | PR [#666](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/666) · `aiContextEnrichment` · `CompanyRegistrySearch.jsx` |
...
| **Libreria → decreti pubblici: XML Akoma Ntoso, non HTML né pdf-to-json (19/09/2026, #688–#690)** — due vie distinte: (1) **seed piattaforma** `legislation_seed` → `norm_requirements` (riga sopra); (2) **import UI** Gestione → Libreria → Normattiva → `document_registry` (`doc_type=decreto`) + `norm_chunks`. Per (2) il download utile è `GET /do/atto/caricaAKN` (XML), non la pagina `!vig=` HTML né skill `pdf-to-json` (quella è per PDF UNI/ISO/Quaderni). **Efficacia ≠ «import OK»**: pezzi (`norm_chunks`) > 0 + articolo noto citabile + ri-domanda all'agente; riga Registro da sola non basta. **Codice**: l'URN Normattiva deve diventare lo stesso `standard_code` del seed (`DLgs_81_2008`, non `urn_nir_stato_…` / `D_Lgs_81_08`) via `publicLawStandardCode.js` + alias; altrimenti quesiti/chat restano su «conoscenze generali» pur avendo il testo in archivio. **Ponte agenti**: catalogo Libreria include `decreto`; quesiti leggono `document_registry`/`norm_chunks` (non `document_type='norm_source'` obsoleto); chat con ISO selezionata **non** scarta i prefissi di diritto pubblico già in `norm_chunks`. **HITL**: non serve per «autorizzare» testo pubblico Normattiva; serve per ISO/UNI a pagamento o quando manca la fonte ufficiale (backlog, non inventare soglie). **Deploy**: nuovo `.js` in `backend/src/` → riga in `deploy-manifest.json` **nella stessa slice** (lezione #690: file già sul VPS, manifest in ritardo). | PR [#688](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/688) · [#689](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/689) · [#690](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/690) · `normattiva.controller.js` · `publicLawStandardCode.js` |
...
| Lezione | Regola da applicare | Dettaglio |
|---------|--------------------|-----------|
| **Accetta su stato init solo in `useEffect` (29/08/2026, SAL AI)** — `SalAiSuggestDialog` metteva `items` solo dopo mount; al primo click Accetta `st.status` era `undefined` → early-return, zero `updateGapStatus` (flake CI sotto carico). | Stato derivato da props già note: init `useState(() => …)` (o sync al mount senza race). Non affidarsi al primo paint post-effect per azioni immediate. Test L1 deve esercitare Accetta subito dopo render. | `SalAiSuggestDialog.jsx` · PR [#598](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/598) |
| **Mock di API che usa `.catch()` deve essere una Promise** — test Accetta conclusioni AI: `aiFeedback` mockato senza `vi.hoisted` + `mockResolvedValue` → `undefined.catch` e `onAccept` non partiva. | `vi.hoisted` per i mock usati nella factory; `mockResolvedValue({})` (o `Promise.resolve` nel componente). `mockClear` non basta se la factory non vede la stessa `vi.fn()`. | `aiConclusionsModal.editable.test.jsx` · PR #498 |
...
| **Suite Cursor `queued` senza check run (25/08/2026, PR #561)** — su PR aperte dal Cloud Agent la suite GitHub `cursor` resta in coda (0 Bugbot / 0 Security Reviewer) anche dopo CI verde. I subagenti `bugbot` hanno trovato bug veri (rete che bloccava A/R/S; eligibility senza `company_access`; metodo UI vs PUT). | «Pronta» = CI **e** Bugbot **e** Security Review **letti** su quello SHA. Suite `queued` ≠ concluso. **Un** run Bugbot a pagamento per PR; secondo solo dopo fix reale; niente terzo. Non mergiare da Cloud Agent. | `.cursor/rules/sgq-git-autonomy.mdc` · PR [#561](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/561) |
| **Merge: Cloud Agent vs bot ProgettoISO (19/09/2026)** — i Cloud Agent Cursor non mergiano mai (vincolo prodotto). Attore operativo: bot Grok «ProgettoISO» via `gh` sotto gate. Alto / migrazioni / deploy VPS / governance = consenso esplicito. | Non confondere i due attori. «Pronta» ≠ Cloud Agent mergia. Security Review assente (piano Ultra) non blocca indefinitamente dopo CI+Bugbot ok. GitHub auto-merge nativo resta opzione del committente. | `.cursor/rules/sgq-git-autonomy.mdc` § Merge da bot ProgettoISO |
...
| Lezione | Regola da applicare | Dettaglio |
|---------|--------------------|-----------|
| **`authorize()` non basta se il controller ribocca i ruoli (20/08/2026)** — superadmin vedeva «Solo admin/auditor possono duplicare template». Il middleware lascia già passare `superadmin`; il secondo `if (['admin','auditor'])` lo escludeva. | Se un controller ripete il controllo ruoli, includere **superadmin** (stesso perimetro di `authorize()`). Non elencare solo admin/auditor. Vale per duplica/carica/elimina template Word. | PR [#503](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/503) · `reportTemplate.controller.js` |
...
| **`deploy-manifest.json` — anche i file `src/config/` vanno elencati (28/07/2026)** — un fix in `src/config/multer.js` (MIME allegati) restava nel repo: il manifest non lo includeva, il VPS continuava a rispondere 415 sui `.tif` e lo smoke post-deploy sembrava smentire il fix. | La regola «ogni nuovo file `.js` in `backend/src/` va nel manifest» vale anche per i **file esistenti mai deployati**: prima di dichiarare che un fix backend non funziona, verificare con `rg <path> backend/scripts/deploy-manifest.json` che il file sia effettivamente nel manifest. `src/config/` ha ora un gruppo dedicato, prima dei controller che lo richiedono. **Conferma 19/09/2026 (#690):** `publicLawStandardCode.js` era già sul VPS via SCP, ma assente dal manifest — aggiornare il manifest **nella stessa slice** del file nuovo, non in un follow-up. | PR #315 · [#690](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/690) |
| **Screening file Office: testo prima, poi 30 righe (20/08/2026)** — Word/Excel sono zip+XML; le prime 30 righe grezze non dicono il tipo. | Estrarre il testo (`mammoth`/`xlsx`), poi campionare 30→90→200 righe (tetto 8 000). Immagini/DWG: solo path. Nuovi utils (`importExtractText.js`, `importProgressiveScreen.js`) **nel manifest** prima del deploy, altrimenti `MODULE_NOT_FOUND`. | PR [#511](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/511) |
| **Import PDF richiede azienda (21/08/2026)** — Ambito «Tutto lo studio» / Patrimonio non sblocca upload né screening. | Scegliere un'azienda cliente sul job. Pulsanti Carica/Estrai/Screening restano visibili, `disabled` + title. BE: 400 `COMPANY_REQUIRED_FOR_UPLOAD` su create/upload/screen. **Annulla caricamento** = elimina job + file, non purge registro. | PR [#514](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/514) · deploy PID `1134642`→`1149359` |
| **Import: un solo controllo azienda = Ambito (21/08/2026)** — due tendine (header + job) confondevano. Tutto lo studio / Patrimonio / omonimo non sono cliente (#428). | Niente tendina «Azienda cliente» sul form job. `company_id` = Ambito se `isClientCompanyId`. `+ Nuovo job` e picker restano visibili, `disabled` + title. Upload su job esistente solo se Ambito === `job.company_id`. | Brief [`DEPUTYTASK.md`](agent-tasks/DEPUTYTASK.md) · `ImportJobsPage.jsx` |
| **Screening norme: posa in 2.3 se l’albero c’è (IA-11, 21/08/2026)** — `resolveNormFolderId` prendeva la prima cartella 2.3 dell’org, non quella dell’azienda. | `commitToRegistry` / `screenAndPlace` usano `resolveFolderByCode(org, '2.3', company_id)`. Albero presente → `parent_id`. Albero assente → coda «Cartella mancante», non 404. Hint job `norma` posa anche a confidence medium. Ingest vero resta **Carica norme** in NORME E LEGGI. | [`PLAN_INGEST_ARCHIVIO_SLICES.md`](agent-tasks/PLAN_INGEST_ARCHIVIO_SLICES.md) · `importJobs.controller.js` |
| **Ingest norme — duplicati e edizioni (IA-15, 22/08/2026)** — stessa famiglia in cartella 2.3 / stessa azienda. `UNI EN 10168` ≡ `EN 10168`. | Stesso anno → `duplicate`, niente seconda riga. Edizione **più nuova** → vigente, precedenti `superata`. Edizione **più vecchia** → `pending_review` + warning, non due vigente; non cancellare. | PR [#532](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/532) · `normIngest` + `standardCodeNormalizer` |
| **Ingest cartella + Gemini TPM (IA-16, 22/08/2026)** — embed in sequenza stretta sfora 1M token/min. Un 429 TPM spegneva la chiave e anche Flash. | Pausa tra PDF (`INGEST_FOLDER_PAUSE_MS`) + batch embed. 429 TPM/rate → retry, **non** `markKeyExhausted`. Esausta solo 403 o quota giornaliera/billing. | PR [#534](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/534) · `geminiKeyPool` + `normUpload` |
| **Timeout ingest cartella 15 min (IA-17, 22/08/2026)** — pause IA-16 allungano `ingest-from-folder`; AbortError a 180s mostrava «Richiesta timeout». | Allineare client + `setTimeout` Node + nginx `proxy_*_timeout` a **900s** solo sul batch norme. Default GET 10–15s resta. `deploy-to-vps.sh` non copia nginx: scp `sgq-backend.conf` + `nginx -t` + reload. | PR [#536](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/536) · `apiService.js` · `normUpload.controller.js` |
| **Coda Documenti «da completare» (IA-5b, 21/08/2026)** — dopo lo screening i file posati restano `ai_draft` / tipo `altro` / senza cartella. Non aprire uno per uno. | **Documenti** → badge **Da completare** (stesso modello Inbox / Qualifiche). Filtro `?tab=catalog&incomplete=1`. Da Import, dopo Screening: «Apri coda da completare». Non è un cancello sullo screening. | [`PLAN_INGEST_ARCHIVIO_SLICES.md`](agent-tasks/PLAN_INGEST_ARCHIVIO_SLICES.md) · `documentIncompleteQueue.js` |
...
| **Rischio con `company_id NULL` invisibile nel filtro azienda (13/09/2026)** — un rischio creato senza `company_id` (bug storico ante-validazione UI) spariva dalla vista quando si filtrava per azienda, pur esistendo nel DB. | Diagnosi: `SELECT * FROM risks WHERE organization_id=X AND company_id IS NULL`. Fix: `UPDATE risks SET company_id=@co WHERE risk_id=@id AND company_id IS NULL` — solo dopo conferma committente sull'azienda corretta. **Monitoraggio futuro**: se un utente segnala «riga mancante» su qualsiasi filtro azienda, verificare prima i record con `company_id IS NULL` nell'org prima di cercare cause più complesse. | `risks` — gap `company_id NULL` derivato dall'assenza del selettore Ambito (vedi riga «Pattern Ambito azienda unico in header» sopra) |
...
| Lezione | Regola da applicare | Dettaglio |
|---------|--------------------|-----------|
| **Simple Browser su HTML statico locale ≠ pagina vuota (23/08/2026)** — Cursor Simple Browser su file HTML locale / `127.0.0.1` mostra vista sorgente «Pretty-print» o pagina bianca anche se il file è pieno. In VM `python` manca. | Per il committente: **Chrome/Edge** + [htmlpreview PT](https://htmlpreview.github.io/?https://github.com/qsstudio241/sistema-gestione-iso9001/blob/main/docs/agent-tasks/spike-cnd-pt-preview.html). Locale agente: `python3 -m http.server`. Lo spike **non è** `/cnd/verbali`; Design Mode non tocca il `.docx`. Certificato = Word Template report (CND-4); PDF dal Word. **Abort** se due layout (HTML+Word) da allineare. | [PLAN CND](agent-tasks/PLAN_CND_SLICES.md) · PR [#548](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/548)/[#550](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/550) |
| **Worktree su disco locale `C:`** | Il repo vive su **Google Drive** (`G:\…`) dietro junction `C:\ProgettoISO`: l'I/O è lento e la **suite Vitest completa si impalla**. Per task corposi/paralleli usare un **worktree su `C:`** da `origin/main`; come L1 affidarsi a **build Vite + Vitest mirato** (o CI Netlify), non alla suite intera. Due finestre Cursor sulla **stessa** directory sono vietate: worktree obbligatorio sul desktop; i Cloud Agent hanno già la VM isolata. | [`sgq-workflow-method.mdc` § 2](../.cursor/rules/sgq-workflow-method.mdc) · [Workspace consigliato](archive/sessions/GUIDA_DIARIO_2026.md#workspace-consigliato--ponte-cprogettoiso-cursor--terminale) |
| **Più chat in parallelo + 1 Bugbot (18/08/2026)** | Sì al multitasking: ogni chat un modulo, file di **codice** disgiunti. **Diari:** GUIDA e roadmap § Stato attuale si incrociano sempre — in parallelo **non** nella PR di codice; traccia nel `DEPUTYTASK*.md` della chat, sync dopo merge (tenere tutte le righe). Chat sola = GUIDA+roadmap nella stessa PR. Desktop = worktree. **Brief:** slot `DEPUTYTASK.md`/`1`/`2` (il numero non è il modulo) o stream `DEPUTYTASK_<EPIC>.md`; non cancellare i CHIUSI; sovrascrivere uno slot solo se già CHIUSO su `main`. **Costo:** Dashboard Bugbot **Run only when mentioned** + un `bugbot run` a lavoro finito. | [`sgq-workflow-method.mdc` § 0 / File di traccia](../.cursor/rules/sgq-workflow-method.mdc) · `sgq-operating-memory.mdc` § Convenzione DEPUTYTASK |
...
| **Smoke TEST 404 ≠ backend TEST spento (20/08/2026)** — `curl :3001/api/v1/health` 200, URL pubblico `/test-api/api/v1/health` 404. Causa: unattended-upgrade nginx `1.24.0-2ubuntu7.16` (06:32) ha spezzato `rewrite … break` nello stesso `location` degli `if` OPTIONS: Node riceveva il path col prefisso. | **Diagnosi:** curl locale vs pubblico. **Fix:** `proxy_pass http://127.0.0.1:3001/;` (slash finale), niente rewrite. Non riavviare TEST se `:3001` è già healthy. | PR [#494](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/494) · `backend/config/nginx/sgq-backend.conf` |
| **Migrazioni DB — sequenza condivisa** | Numerazione **unica** (stato ~154). Le PR vecchie con numeri bassi vanno **rinumerate in coda** e rese **idempotenti** (check esistenza prima di `ALTER`/`CREATE`). FK SQL Server: statement separati. Due Cloud Agent non prenotano lo stesso numero. | [how-to/database-migrations.md](how-to/database-migrations.md) · [`sgq-workflow-method.mdc` § 6](../.cursor/rules/sgq-workflow-method.mdc) |
...
| **PDF → Markdown → JSON (tool generico)** | Per digitalizzare una nuova norma/Quaderno/checklist/capitolato: usare **sempre** `backend/scripts/pdf_to_json/` (pdfplumber + fallback pymupdf/pypdf, OCR locale opzionale via tesseract, nessuna chiamata cloud) invece di scrivere parsing ad-hoc. Rileva anche i PDF con font a codifica rotta/offuscata (testo "presente" ma illeggibile, es. placeholder `(cid:NNN)`) e li segnala come pagine a bassa qualità invece di produrre JSON silenziosamente sbagliato. Salva sempre il `.md` intermedio da revisionare prima di fidarsi del `.json`. **Non** usarlo per decreti/leggi da Normattiva: lì l'import Libreria scarica XML Akoma Ntoso (`caricaAKN`) — vedi lezione *Libreria → decreti pubblici* sopra. | [`.cursor/skills/pdf-to-json/`](../.cursor/skills/pdf-to-json/SKILL.md) · [`backend/scripts/pdf_to_json/README.md`](../backend/scripts/pdf_to_json/README.md) |
| **Tavole vettoriali (ISO 2553) — `get_images()` da solo non basta (MR-0, 18/08/2026)** | Le piastre di simboli saldatura sono spesso **disegni vettoriali**, non JPEG. Flag `--extract-figures` (default off): pymupdf `get_images` + bbox di pagina **e** cluster di `get_drawings()`, PNG + `*.figures.json`, tutto locale. Scartare rumore (linee isolate, footer). **Estendere** `pdf_to_json`, non sostituirlo; niente secondo vector DB in questa epic. PDF norma reale = copyright: **non** in Git (fixture ReportLab nei test). Catalogo `weldingSymbols2553.js` resta la fonte codici; il RAG visivo lo affianca (MR-1 in poi). | [PLAN](agent-tasks/PLAN_MULTIMODAL_RAG_SLICES.md) · PR [#464](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/464) |
| **Citazioni tavola = stesso pannello Assistente (MR-2, 19/08/2026)** — query testo → ritaglio locale (pagina + bbox). Niente galleria di prodotto. | Riusare `AiAssistantCitations`. GET `/ai/figures/:id/image` sullo stesso controller figure. **AI cita, non certifica** (ADR-010). Nessun Gemini sui PNG. Crop locale. | PR [#475](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/475) · [PLAN](agent-tasks/PLAN_MULTIMODAL_RAG_SLICES.md) |
| **Ingest figure = extract locale + persist CLIP (MR-3, 19/08/2026)** — PDF già sul disco, stesso tenant. | `POST /ai/figures/ingest`: path autorizzato → extract MR-0 + `persistFigures` CLIP. Org solo dal JWT. PDF senza tavole → lista vuota. Niente Gemini sui PNG. **AI cita, non certifica.** | PR [#484](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/484) · [PLAN](agent-tasks/PLAN_MULTIMODAL_RAG_SLICES.md) |
| **Query visiva = ritaglio nello stesso spazio CLIP (MR-4, 19/08/2026)** — PNG/JPEG/WebP, top-k tavole, stesso pannello Assistente. | `POST /ai/figures/search-by-image` (multipart `file`). Org solo dal JWT. Senza file → 400; nessun match → `{ figures: [] }` 200. Bottone ritaglio nel composer. Niente Gemini sui PNG. **AI cita, non certifica.** Mig. **154** già su TEST e PROD prima del deploy codice. | PR [#489](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/489) · [PLAN](agent-tasks/PLAN_MULTIMODAL_RAG_SLICES.md) |
| **Commit norma → tavole CLIP (FW-0, 20/08/2026)** — senza questo hook Ask+immagine in Assistente resta vuoto. | `commitNormFromFields` chiama `ingestFiguresFromPdf` (path + org JWT + `companyId` cartella). Throw CLIP → log, norma salvata. DELETE figure isolato per azienda. **Non** collegare WPQR. | PR [#494](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/494) · [PLAN](agent-tasks/PLAN_FIGURE_WPQR_SLICES.md) |
...
| **Import PDF batch (Sprint 9)** | Tabelle `import_jobs`, `import_job_files`; API `GET/POST/PATCH/DELETE /import-jobs`, upload `POST .../files` (multipart `files` + `relative_paths`), `POST .../process` estrae testo da PDF/Word/Excel/txt (`importExtractText`) + `confidenceFromTextLength`. **`POST .../files/:fileId/ai-extract`**: estrazione JSON strutturata via OpenAI sul testo già estratto (richiede `OPENAI_API_KEY` sul server; rate limit dedicato). Colonne file: `ai_extraction_json`, `ai_extraction_error`, `ai_extraction_at`, `ai_model` (migrazione **039**). Licenza modulo **`ai_import`**. UI admin: **Impostazioni → Import PDF** (`/settings/import-jobs`). **IA-4/IA-5 + cartella tutti i tipi (20/08, #506/#509/#511)**: «Carica cartella» (`webkitdirectory`); path relativo in `original_name` (niente colonna nuova, max 80 file); screening **30→90→200 righe** (`importProgressiveScreen`); posa in scaffale solo se confidence alta + azienda + tipo non-qualifica. Senza azienda: `400 COMPANY_REQUIRED_FOR_UPLOAD` (create/upload/screen — #514). Ambito «Tutto lo studio»/Patrimonio non sblocca. «Annulla caricamento» = elimina job + file (non purge registro). Disegni/foto: solo nome/cartella (OCR = IA-8). Deploy VPS: manifest include `importExtractText.js`, `importProgressiveScreen.js`, `importRelativePath.js`, `importScreening.js`, `importJobs.controller.js` + routes + restart. **Privacy**: il testo inviato all’API è lo stesso mostrato in schermata revisione; valutare accordo/DPA OpenAI per l’organizzazione. |
...
> **Nota porta 8444**: nginx è configurato anche su `:8444` (TLS) → `:3001`, ma il provider non espone quella porta all'esterno. Si accede via path-prefix `/test-api/` sulla porta `8443` già aperta. Lo strip del prefisso è `proxy_pass http://127.0.0.1:3001/;` (slash finale) in `sites-available/sgq-backend` — **non** `rewrite … break` nello stesso `location` degli `if` OPTIONS (rotto dopo nginx 7.16, 20/08/2026).
...
- **Upload / Duplica template (20/08/2026, #501)**: archivio unico = **VPS**. Catalogo in `report_templates`; file in `backend/templates/` (sistema) o `/uploads/…` (studio). Export e download passano da `GET /api/v1/report-templates/:id/file` (auth). `app/public/templates/` su Netlify resta solo **fallback offline**. Copiare un `.docx` in `public/templates/` **non** registra il template e **non** basta per Duplica. **Da UI**: Gestione → Template report → **Duplica** sulla riga (modal nome) oppure Carica file; assegnazione ISO sotto griglia, checklist custom in Checklist personalizzate. Warning soft se mancano `CHECKLIST_MARKER` / `RILIEVI_MARKER`. API: `POST /report-templates/:id/duplicate` `{ name }`.
- **Intestazione verbale**: modifiche grafiche (logo, layout) sul modello di sistema in `backend/templates/VerbaleVisita-generic.docx` (stessa copia in repo `app/public/templates/` per fallback) + deploy VPS; runtime OOXML non sostituisce l’header se già nel template patchato.
...
**Registrazione template custom (menu a tendina)**: il dropdown in **Admin → Checklist personalizzate → editor** legge `GET /report-templates?scope=audit` (righe in tabella `report_templates`: template di sistema `organization_id` NULL + upload/duplicati org). Copiare/rinominare un file sotto `app/public/templates/` **non** crea una voce nel menu. Per una copia del verbale ISO 9001: **Duplica** (o Carica) da Gestione → Template report, poi **PUT** `/report-template-assignments/custom-checklist/:id` (o dropdown nell'editor). Il file deve contenere i marker `CHECKLIST_MARKER` e `RILIEVI_MARKER` oltre ai placeholder docxtemplater (`{auditDate}`, `{clientName}`, …).
...
- **ISO-7 (19/08/2026, mergiata #474)**: `rdp_reports.project_id` + `ndt_reports.project_id` nullable, FK `ON DELETE SET NULL`. Runner `run-migration-155-vps.js` (`SGQ_MIGRATION_TARGET=test` prima). Helper `resolveOptionalProjectId`.
...
- **MC-3 (17/08/2026)**: Rule Engine `materialComplianceRuleEngine.service.js` — input JSON + snapshot KB, output `pass|fail|skip` + `checks[]`. Zero LLM. Non imposta `workflow_status=compliant` (solo HITL). Più restrittivo vince (ADR-021) da `scope.po|customer|company` iniettato, niente `if (cliente === 'FASSI')`. Migrazione: nessuna (schema 149 basta). PR [#454](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/454).
- **MC-4 (17/08/2026, mergiata 18/08 #456)**: API `/api/v1/material-certificates` — lista/dettaglio/upload, extract (riuso ingest, OCR assente → `ocr_skipped`), evaluate persiste checks e passa a `pending_review`. `compliant` solo da POST approve. Licenza AND `saldatura`+`ai_import`. Nessuna migrazione nuova.
