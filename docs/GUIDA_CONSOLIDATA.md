...
| **Compliance Map ≠ SAL ≠ Second Brain (06/09/2026)** — metodo wiki = grafo requisito↔fonte↔gap **in SQL**, non vault. | Mappa = grafo capitolato scoped org+company, versionato, HITL. SAL = maturità SGQ clausole×azienda. Second Brain = fatti live (conteggi) fuori dal prompt. Catalogo `norm_requirements` resta piattaforma; i link stanno negli items. Codice CM-1 solo dopo SB-4 su `main`. | [`PLAN_COMPLIANCE_MAP_SLICES`](agent-tasks/PLAN_COMPLIANCE_MAP_SLICES.md) · [#652](https://github.com/qsstudio241/sistema-gestione-iso9001/pull/652) |
