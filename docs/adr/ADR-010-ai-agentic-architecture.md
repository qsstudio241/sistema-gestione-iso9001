...
| UNI Store scraper (Task 1-E) | **Testato — parzialmente fattibile con login assistito** (05/07/2026) | Login **completamente automatico**: bloccato da anti-bot (nessuna risposta HTTP alla chiamata di auth). Login **assistito** (browser visibile, credenziali precompilate, click eseguito da una persona reale): **funziona** — JWT ricevuto, sessione riutilizzabile via `context.storageState()` (niente re-login ad ogni richiesta). Dopo il login, la navigazione automatica alla scheda della norma **funziona** (titolo/metadati corretti). Limite: se la norma richiede acquisto singolo (non inclusa nell'abbonamento), la scheda mostra un flusso di **acquisto** — da **non automatizzare mai** (rischio addebito reale). Anche a download avvenuto, il PDF resta **DRM FileOpen**: illeggibile da script senza il visualizzatore proprietario. Script: `backend/scripts/uni-store-diagnostic.js` (env `UNI_STORE_HUMAN_LOGIN=true`), non wired in produzione. **Prossimo passo utile**: verificare se una norma già inclusa nell'abbonamento (non acquisto singolo) espone un lettore "consultazione integrale" con testo leggibile via DOM, invece del flusso di acquisto testato finora. Percorso alternativo sempre valido: `ManualUploadFallback` — download manuale autorizzato + conversione in `docs/Normative/*.md`/pipeline PDF esistente. |
