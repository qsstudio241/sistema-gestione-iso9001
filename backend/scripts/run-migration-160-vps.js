/**
 * Migration 160 (VPS) — library_source_requests (LG-1 Gap fonti).
 *
 *   scp -P 1122 database/migrations/160_library_source_requests.sql \
 *     spascarella@sistemi.fr-busato.it:/var/www/sgq-backend/database/migrations/
 *   scp -P 1122 backend/scripts/run-migration-160-vps.js \
 *     spascarella@sistemi.fr-busato.it:/tmp/
 *   ssh -p 1122 spascarella@sistemi.fr-busato.it 'node /tmp/run-migration-160-vps.js'
 */
const fs = require('fs');
...
const SQL_CANDIDATES = [
  `${BACKEND_ROOT}/database/migrations/160_library_source_requests.sql`,
  '/tmp/160_library_source_requests.sql',
];
...
function resolveSqlPath() {
  ...
  if (!found) {
    throw new Error(
      'SQL 160 non trovato. Copia 160_library_source_requests.sql in ' +
...
async function run() {
  ...
  try {
    console.log(`[160] target=${IS_TEST ? 'test' : 'prod'} SQL: ${sqlPath} — ${steps.length} step`);
    for (let i = 0; i < steps.length; i++) {
      await pool.request().query(steps[i]);
      console.log(`[160] Step ${i + 1}/${steps.length} OK`);
    }
    const tab = await pool.request().query(`
      SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'library_source_requests'
    `);
    if (!(tab.recordset || []).length) {
      console.error('[160] ERRORE: tabella library_source_requests assente');
      process.exitCode = 1;
      return;
    }
    console.log('[160] Migration completata.');
  } catch (e) {
    console.error('[160] ERRORE:', e.message);
    process.exitCode = 1;
  } finally {
