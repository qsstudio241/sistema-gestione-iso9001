/**
 * Migration 152 (VPS) — risk_reviews (ROO-16).
 * Ex 150 su TEST: rinumerata dopo collisione con MC-1 / 149.
 *
 *   scp -P 1122 database/migrations/152_risk_reviews.sql \
 *     spascarella@sistemi.fr-busato.it:/var/www/sgq-backend/database/migrations/
 *   scp -P 1122 backend/scripts/run-migration-152-vps.js \
 *     spascarella@sistemi.fr-busato.it:/tmp/
 *   ssh -p 1122 spascarella@sistemi.fr-busato.it 'node /tmp/run-migration-152-vps.js'
 *   ssh -p 1122 ... 'SGQ_MIGRATION_TARGET=test node /tmp/run-migration-152-vps.js'
 */
const fs = require('fs');
...
const SQL_CANDIDATES = [
  `${BACKEND_ROOT}/database/migrations/152_risk_reviews.sql`,
...
function resolveSqlPath() {
  ...
  if (!found) {
    throw new Error('SQL 152 non trovato. Copia 152_risk_reviews.sql in ' + SQL_CANDIDATES.join(' oppure '));
  }
  return found;
}
...
async function run() {
  ...
  try {
    console.log(`[152] target=${IS_TEST ? 'test' : 'prod'} SQL: ${sqlPath} — ${steps.length} step`);
    for (let i = 0; i < steps.length; i++) {
      await pool.request().query(steps[i]);
      console.log(`[152] Step ${i + 1}/${steps.length} OK`);
    }
    const tab = await pool.request().query(`
      SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'risk_reviews'
    `);
    if (!(tab.recordset || []).length) {
      console.error('[152] ERRORE: tabella risk_reviews assente');
      process.exitCode = 1;
      return;
    }
    console.log('[152] Migration completata.');
  } catch (e) {
    console.error('[152] ERRORE:', e.message);
    process.exitCode = 1;
  } finally {
