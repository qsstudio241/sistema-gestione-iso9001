/**
 * Migration 106 — ndt_reports + ndt_report_items + ndt_report_instruments
...
async function run() {
    ...
    try {
        for (const step of STEPS) {
            console.log(`[106] Eseguo: ${step.name}`);
            const result = await pool.request().query(step.sql);
            if (result.recordset && result.recordset.length > 0) {
                console.log('     Output:', result.recordset);
            }
        }
        console.log('[106] Migration ndt_reports + items + instruments completata.');
    } catch (e) {
        console.error('[106] ERRORE:', e.message);
        process.exit(1);
    } finally {
