/**
 * Migration 116 (local) — Persistenza analisi capitolato: source su ccde + req_type esteso su ccer.
 * Esegue la stessa logica idempotente della 116-vps usando backend/config/database.json.
 * Statement separati (no batch unico) per evitare "Invalid column name 'source'" in fase di compile.
 *
 * Uso (da Windows):
 *   node scripts/run-migration-116-local.js test         # DB di test (default consigliato)
 *   node scripts/run-migration-116-local.js production   # DB di produzione
 */
...
async function main() {
    ...
    console.log(`Migration 116 → [${c.database}] su ${c.server}:${c.port}`);
    ...
    console.log('  OK: migration 116 applicata.');
    await pool.close();
    process.exit(0);
}

main().catch((e) => { console.error('ERRORE migration 116:', e.message); process.exit(1); });
