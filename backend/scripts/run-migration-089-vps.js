/**
 * Migration 089: Schema completo welding_procedures, wpqr_records, wps_welders
 * Eseguire su VPS: node /tmp/run-migration-089-vps.js
 */
'use strict';
...
const SQL_BATCHES = [
    // Crea welding_procedures se non esiste
...
END`,
    ...
    // Aggiungi colonne mancanti a welding_procedures
    ...
    // Allinea material_group ? base_material_group
    `UPDATE welding_procedures SET base_material_group = material_group
    WHERE base_material_group IS NULL AND material_group IS NOT NULL`,

    // Crea wpqr_records se non esiste
...
END`,
    ...
    // Allinea testing_body ? examiner_body e wpqr_code ? reference_number
    `UPDATE wpqr_records SET examiner_body = testing_body WHERE examiner_body IS NULL AND testing_body IS NOT NULL`,
    `UPDATE wpqr_records SET reference_number = wpqr_code WHERE reference_number IS NULL AND wpqr_code IS NOT NULL`,

    // Crea wps_welders se non esiste
...
async function run() {
    console.log('[089] Connecting to SQL Server...');
    const pool = await mssql.connect(DB_CONFIG);
    for (let i = 0; i < SQL_BATCHES.length; i++) {
        console.log(`[089] Batch ${i + 1}/${SQL_BATCHES.length}...`);
        try {
            const res = await pool.request().query(SQL_BATCHES[i]);
            if (res.recordset) console.log('  result:', JSON.stringify(res.recordset));
        } catch (err) {
            console.error(`  ERRORE batch ${i + 1}:`, err.message);
            throw err;
        }
    }
    console.log('[089] Migration completata OK');
    process.exit(0);
}

run().catch((err) => {
    console.error('[089] ERRORE FATALE:', err.message);
    process.exit(1);
});
