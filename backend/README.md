...
│  https://busato.selfip.com:10443/api/v1          │
...
- 20 GB spazio disco
- IP pubblico statico
- Dominio: `busato.selfip.com` puntato al server
...
sudo ls -la /etc/letsencrypt/live/busato.selfip.com/
```
...
server {
    ...
    server_name busato.selfip.com;

    # SSL Certificates
    ssl_certificate /etc/letsencrypt/live/busato.selfip.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/busato.selfip.com/privkey.pem;
...
```bash
curl -k https://busato.selfip.com:10443/health
```
...
```bash
curl -k -X POST https://busato.selfip.com:8443/api/v1/auth/register \
...
```bash
curl -k -X POST https://busato.selfip.com:8443/api/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{
    "email": "test@qsstudio.it",
    "password": "<password-di-test>"
...
- **Documentazione API**: https://busato.selfip.com:8443/api-docs (da implementare con Swagger)
