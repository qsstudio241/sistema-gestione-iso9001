...
- joint_type: "BW", "FW", "BW+FW" oppure "SW" (Stud / prigioniero — tipologia dedicata, DISTINCT da FW).
  Se il verbale parla di stud welding, prigioniero, arc stud, ISO 14555, oppure il pezzo di prova e' un
  prigioniero/stud (D1, Parent Metal 2 = stud), imposta joint_type: "SW" ANCHE se il giunto e' fillet/angolo.
  NON usare FW per lo stud. NON inventare range di validita ISO 14555 (niente formule, solo numeri scritti).
- product_type: "P" (piastra), "T" (tubo) oppure "P+T" (entrambi) - variabile essenziale ISO 15614-1 par.8.3.3 per il diametro.
  ...
  Un prigioniero cilindrico su piastra NON e' P+T: resta SW + diametro prigioniero, product_type di solito "P".
...
- material_group_2: gruppo materiale Parent Metal 2 (es. prigioniero) se il verbale dichiara DUE materiali/gruppi
  (Parent Metal 1 + Parent Metal 2); null se uno solo. Copiare il gruppo del PM2, non duplicare a caso il PM1
  se il verbale indica un solo gruppo
...
- thickness_max_unlimited: booleano — true SOLO se il verbale dichiara esplicitamente un range aperto
  ...
  le due situazioni
- thickness_t1_min / thickness_t1_max / thickness_t1_max_unlimited: se il verbale dichiara DUE range
...
- diameter_min / diameter_max: range diametro — per tubo (product_type T/P+T) oppure diametro del PRIGIONIERO
  se joint_type=SW (D1, Ø stud; NON il diametro tubo ISO 15614-1 par.8.3.3).
  SOLO se un numero e' dichiarato - vedi nota su product_type sopra per il caso testo/piastra.
  Se SW e manca il numero, lascia null: NON calcolare un range ISO 14555
