 ...
 * Indicizza tutte le entità SGQ (audit, NC, reclami, qualifiche, rischi, documenti)
...
const INDEXABLE_ENTITIES = [
  ...
  },
  {
    ...
    buildText: (r) =>
      `NC ${r.nc_number || ''} (${r.nc_type || '?'}): ${r.description || ''}. Gravità: ${r.severity || '?'}, Stato: ${r.status || '?'}`,
  },
  {
  ...
  },
  {
    ...
    buildText: (r) => {
      ...
      if (r.severity) parts.push(`Gravità: ${r.severity}`);
  ...
  },
  {
    ...
    buildText: (r) => {
      ...
      parts.push(`Probabilità: ${r.probability}, Impatto: ${r.impact}, Score: ${score}`);
...
/**
 * Indicizza tutte le entità SGQ per un'organizzazione.
 */
